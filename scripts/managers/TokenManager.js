...
class TokenManager {
	...
	_updateActiveToken() {
		...
		if (!game.user.isGM) {
			...
			if (controlled.length > 0) {
				...
				const characterToken = canvas.tokens.placeables.find(t => t.document.actorId === game.user.character.id);
		...
		// Use Hooks.callAll with proper typing - this is a custom hook so TypeScript warnings are expected
		// @ts-ignore - Custom hook not in Foundry's type definitions
	...
	getActiveToken() {
		...
		if (this.activeToken && canvas.tokens.placeables.includes(this.activeToken)) {
