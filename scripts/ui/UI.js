...
export class DebuggerUIBuilder {
  ...
  static _createEffectPointGroupsHTML(effectKey, options = {}) {
    ...
    const allGroups = mgr?.getGroups ? mgr.getGroups() : {};
  ...
  _getLightingHTML() {
    ...
    `;
    return DebuggerUIBuilder._createAccordionHTML(effectKey, "Lighting", content);
  }
  ...
  _getPointGroupsHTML() {
    ...
    const createGroupListHTML = (groups) => {
      ...
      return groups
        .map((group) => `
          <details class="group-list-item">
            <summary>
              <span class="accordion-toggle"></span>
              <span class="mp-group-item-status ${group.isBroken ? "broken" : "valid"}" title="${group.isBroken ? group.reason : "Valid"}"></span>
              <span class="group-item-label">${Handlebars.escapeExpression(group.label)}</span>
              <span class="group-item-meta">${group.points.length} pts</span>
              <button type="button" class="group-delete-btn" data-action="delete-group" data-group-id="${group.id}" title="Delete this group">
                <i class="fas fa-trash"></i>
              </button>
            </summary>
            <div class="group-item-content">
                    ...
                    .map(
                      (p, i) => `
                        <div class="point-item">
                          ...
                          <span class="point-coord">X: ${Math.round(p.x)}</span>
                          <span class="point-coord">Y: ${Math.round(p.y)}</span>
                          <button type="button" class="point-delete-btn" data-action="delete-point" data-group-id="${group.id}" data-point-index="${i}" title="Delete point">
                            <i class="fas fa-times"></i>
                          </button>
                        </div>
                      `
    ...
    const content = `
      ...
      <details open class="create-group-accordion">
        <summary><span class="accordion-toggle"></span>Create New Group</summary>
        <div class="create-group-content">
          ...
          <button type="button" class="create-group-btn" data-action="create-group-from-ui"><i class="fas fa-plus-square"></i> Create Group</button>
        </div>
      </details>
      <details open class="group-type-accordion">
        <summary><span class="accordion-toggle"></span><strong>Point Groups</strong></summary>
        <div class="group-type-content">${createGroupListHTML(groupsByType.point)}</div>
      </details>
      <details class="group-type-accordion">
        <summary><span class="accordion-toggle"></span><strong>Line Groups</strong></summary>
        <div class="group-type-content">${createGroupListHTML(groupsByType.line)}</div>
      </details>
      <details class="group-type-accordion">
        <summary><span class="accordion-toggle"></span><strong>Area Groups</strong></summary>
        <div class="group-type-content">${createGroupListHTML(groupsByType.area)}</div>
      </details>
      <details class="group-type-accordion">
        <summary><span class="accordion-toggle"></span><strong>Rope Groups</strong></summary>
        <div class="group-type-content">${createGroupListHTML(groupsByType.rope)}</div>
      </details>
    `;

    return DebuggerUIBuilder._createAccordionHTML("pointGroups", "Point Groups", content);
  }
...
export class DebuggerEventHandler {
    ...
    if (stats) {
      ...
      if (methodName === 'updateAllControls' && stats.lastCallTime) {
        ...
        if (timeSinceLastCall < 100) {
          console.warn(`🔴 Event Handler | ${methodName} called too frequently! ${timeSinceLastCall}ms since last call`);
        }
      }
      ...
      const threshold = this._perfThresholds[methodName] || 10;
      if (duration > threshold) {
        console.warn(`🔴 Event Handler | ${methodName} took ${duration.toFixed(2)}ms (threshold: ${threshold}ms)`);
  ...
  initialize() {
    ...
    this._boundGradientBarDoubleClick =
      this._onGradientBarDoubleClick.bind(this);
  ...
  async _createParticleEffectArea(effectKey) {
    ...
    // Use MapPointsManager class directly (static methods)
    if (!MapPointsManager?.createGroup) {
      ui.notifications.error("Map Shine | Map Points Manager is not available yet. Try again after the scene finishes loading.");
      return;
    }
  ...
  async _createPhysicsRope(ropeType = "rope") {
    ...
    const newGroupId = await mgr.createGroup({
