...
class ResourceManager {
	...
	getCloudShadowTexture(deltaTime) {
		...
		const texture = cloudLayer.cloudShadowTexture;
		...
		this.element.innerHTML = `
                        ...
                        <style>
                            #map-shine-loading-screen { 
                                ...
                                transition: opacity ${this.fadeOutDuration / 1000
			}s ease-in-out; 
                            }
...
class MapShineLifecycle {
	...
	static async runMinimalSetup(canvas) {
		...
		if (game.mapShine.resolveSetupCompletion) {
			console.log(
				"Map Shine | Signaling minimal setup completion to scene transition."
			);
			game.mapShine.resolveSetupCompletion();
		}
		...
		for (let i = 0; i < points.length; i++) {
			...
			for (let j = i + 2; j < points.length; j++) {
				...
				if (this._checkIntersection(p1, p2, p3, p4)) {
					group.isBroken = true;
					group.reason = `Segment ${i + 1}-${i + 2} intersects segment ${j + 1
						}-${j + 2}.`;
					return group;
				}
			}
        ...
        `;
          ...
          <style>
            ...
            .map-shine-pause-wrapper::after {
              ...
              right: -50vw;
              height: 3px;
              background: linear-gradient(to right, transparent, ${settings.gradientColor1
					}, ${settings.gradientColor2}, ${settings.gradientColor1
					}, transparent);
            }
            ...
            .map-shine-pause-title {
              ...
              font-size: 4em; margin: 0; letter-spacing: 5px; color: ${settings.headingColor
					}; text-transform: uppercase;
              text-shadow: 0 0 10px #000;
            }
            .map-shine-pause-subtitle {
              font-family: "${settings.subheadingFont}", sans-serif;
              font-size: 1.5em; margin: 10px 0 20px 0; color: ${settings.subheadingColor
					}; font-style: italic;
              text-shadow: 0 0 10px #000;
            }
            .map-shine-pause-logo {
              ...
              margin: 0 auto; opacity: ${settings.logoOpacity
            ...
            @keyframes pulseLogo {
              0%, 100% { transform: scale(1); opacity: ${settings.logoOpacity
					}; }
		...
		if (effectKey === "sparks") {
			...
			let sparksContent = `
					...
					0.1
									  ...
									  ${DebuggerUIBuilder._createGradientEditorHTML(`${sparksPath}.colorAlphaGradient`, "Color & Alpha Over Life")}
									  ${DebuggerUIBuilder._createGradientEditorHTML(`${sparksPath}.emissiveGradient`, "Emissive (Brightness) Over Life")}
	...
	_buildHTML(data) {
		...
		if (selectedGroup) {
			const effectOptions = Object.entries(EFFECT_SOURCE_OPTIONS)
				.map(
					([key, name]) =>
						`<option value="${key}" ${selectedGroup.effectTarget === key ? "selected" : ""
						}>${name}</option>`
				)
				.join("");
				...
				selectedGroup.label
                                ...
                                <div class="control-row">
                                    ...
                                    <select name="type" id="mp-group-type">
                                        <option value="point" ${selectedGroup.type === "point"
					? "selected"
					: ""
				}>Points</option>
                                        <option value="line" ${selectedGroup.type === "line"
					? "selected"
					: ""
				}>Line</option>
                                        <option value="area" ${selectedGroup.type === "area"
					? "selected"
					: ""
				}>Area</option>
                                    </select>
					...
					.join("")}
                            ...
                            <div class="mp-effect-source-settings">
                                ...
                                <div class="control-row">
                                    ...
                                    <input type="checkbox" name="isEffectSource" id="mp-isEffectSource" ${selectedGroup.isEffectSource
					? "checked"
					: ""
				}>
                                </div>
                                ...
                                <div id="mp-emission-settings-wrapper" style="margin-top: 10px; display: ${selectedGroup.isEffectSource
					? "block"
					: "none"
					...
					1
                                    ...
                                    <div id="mp-emission-falloff-wrapper" style="margin-top: 5px; display: ${selectedGroup.type === "point"
					? "none"
					: "block"
				};">
                                        <div class="control-row">
                                            ...
                                            <input type="checkbox" name="emission.falloff.enabled" id="mp-emission-falloff-enabled" ${emission.falloff.enabled
					? "checked"
					: ""
				}>
                                        </div>
                                        <div class="control-row control-row-slider" style="display: ${emission.falloff.enabled
					? "grid"
					: "none"
				};">
		...
		return `
                        <style>
                            ...
                            .mp-editor .control-row-slider { display: grid; grid-template-columns: auto 1fr auto; gap: 5px; align-items: center; }
                            .mp-editor .control-row-slider .value-span { width: 35px; text-align: right; font-family: monospace; background: rgba(0,0,0,0.4); padding: 2px 4px; border-radius: 3px;}
                            ...
                            .mp-effect-source-settings details[open] { background: rgba(255,255,255,0.08); padding-bottom: 5px; }
                            .mp-effect-source-settings summary { font-weight: bold; cursor: pointer; padding: 2px; display: flex; align-items: center; gap: 5px; list-style: none; }
                            .mp-effect-source-settings summary .summary-control { display: flex; justify-content: space-between; align-items: center; width: 100%; }
				...
				.map(
					(g) => `
                                            <li class="mp-group-item ${g.id === this._selectedGroupId
							? "selected"
							: ""
						}" data-group-id="${g.id
						}" data-action="select-group">
                                                <span class="mp-group-item-status ${g.isBroken
							? "broken"
							: "valid"
						}" title="${g.isBroken ? g.reason : "Valid"
						}"></span>
                                                <span class="mp-group-item-label">${Handlebars.escapeExpression(
							g.label
						)}</span>
                                                <span class="mp-group-item-type">${g.type
						}</span>
                                            </li>
                                        `
				)
				...
				if (this._selectedGroupId) {
					...
					Dialog.confirm({
						...
						yes: async () => {
							...
							// State will be reset automatically by the re-render triggered by the hook
						},
						defaultYes: false,
					});
				}
		...
		const fragmentSrc = `
                        ...
                        void main() {
                            ...
                            if (vScreenCoord.x < sceneMin.x || vScreenCoord.x > sceneMax.x || vScreenCoord.y < sceneMin.y || vScreenCoord.y > sceneMax.y) {
                                if (u_outputHighlightMask || u_outputRawCloud) {
...
class GroundGlowLayer extends CanvasLayer {
		...
		return DebuggerUIBuilder._createAccordionHTML(
			...
			`
				...
				0.01
                        ...
                        ${DebuggerUIBuilder._createSliderHTML(
				"groundGlow.luminanceThreshold",
				"Light Threshold",
				0,
				1,
				0.01,
				"The scene brightness level above which the glow will fade out."
			)}
                        ${DebuggerUIBuilder._createSliderHTML(
				"groundGlow.softness",
				"Edge Softness",
	...
	_onAnimate() {
		...
		// This layer is now inert as its sole dependency (illumination texture) has been removed.
		// The class structure is kept to minimize breakage, but it performs no actions.
	...
	_onResize() {
		this.maskGenerator?.resize(
			canvas.app.screen.width,
			canvas.app.screen.height
		);
	...
	_getHTML() {
		...
		return `
            <style>
                ...
                .clock-controls .time-display-input {
                    width: 60px; height: 30px; text-align: center; font-size: 1.1em; background: #2a2a2a; color: white; border: 1px solid #666; border-radius: 4px;
                }
                .clock-disclaimer {
                    font-size: 11px;
                    color: #aaa;
            ...
            <div class="day-night-clock-component">
                ...
                <div class="clock-container">
                    <div class="clock-face" style="--clock-tint-color: ${initialTintColor};">
                        ...
                        <div class="clock-hand"><img class="clock-icon" src="${initialIcon}"></div>
                    </div>
                </div>
...
class TimeOfDayColorFilter extends PIXI.Filter {
	constructor(options = {}) {
		const fragmentSrc = `
            ...
            // "From" Keyframe Uniforms
            uniform float uFromSaturation;
            uniform float uFromBrightness;
            uniform float uFromContrast;
            uniform float uFromExposure;
            uniform float uFromGamma;
            uniform float uFromTemperature;
            uniform float uFromTint;

            // "To" Keyframe Uniforms
            uniform float uToSaturation;
            uniform float uToBrightness;
            uniform float uToContrast;
            uniform float uToExposure;
            uniform float uToGamma;
            uniform float uToTemperature;
            uniform float uToTint;
            ...
            void main(void) {
                ...
                // Get the straight, non-premultiplied color for calculations
                vec3 straightColor = originalColor.rgb;
                if (originalColor.a > 0.0) {
                    straightColor /= originalColor.a;
                }
                
                vec3 workingColor = straightColor;

                // --- 1. Interpolate all parameters ---
                float saturation = mix(uFromSaturation, uToSaturation, uBlendFactor);
                float brightness = mix(uFromBrightness, uToBrightness, uBlendFactor);
                float contrast = mix(uFromContrast, uToContrast, uBlendFactor);
                float exposure = mix(uFromExposure, uToExposure, uBlendFactor);
                float gamma = mix(uFromGamma, uToGamma, uBlendFactor);
                ...
                vec3 finalStraightColor = mix(straightColor, workingColor, uIntensity * outdoorsMask);
                
                // --- 4. Re-premultiply the alpha ---
                vec3 finalPremultiplied = clamp(finalStraightColor, 0.0, 1.0) * originalColor.a;
                
                gl_FragColor = vec4(finalPremultiplied, originalColor.a);
            }
        `;
		...
		console.log(
			`%c[MapShine TimeOfDay]`,
			"color: #800080; font-weight: bold;",
			"TimeOfDayColorFilter instance created."
		);
	}
}

class TimeOfDayLayer extends MaskedEffectLayer {
	constructor() {
		super({
			maskSuffix: "outdoors",
		});
...
class CurveEditor {
	...
	drawCurve() {
		...
		const pathData = `M ${p[0].x},${this.height - p[0].y} C ${p[1].x},${this.height - p[1].y
		...
		if (hasSceneProfiles) {
			...
			contentHTML = `
                <div class="main-controls-grid">
                    <div class="main-controls-col">
                        ...
                        <div class="profile-select-wrapper">
                            ...
                            <div class="profile-management-buttons">
                                <button data-action="rename-scene-profile" title="Rename Selected Profile" ${!isGm ? "disabled" : ""
				}><i class="fas fa-edit"></i></button>
                                <button data-action="delete-scene-profile" title="Delete Selected Profile" ${!isGm ? "disabled" : ""
				}><i class="fas fa-trash"></i></button>
                            </div>
                        </div>
                        <div class="new-profile-controls">
                           <input type="text" id="new-scene-profile-name" placeholder="New Appearance Name..." ${!isGm ? "disabled" : ""
				}>
                           <button data-action="save-scene-profile" id="save-scene-profile-btn" title="Save the current settings as a new scene appearance profile. (GM Only)" ${!isGm ? "disabled" : ""
                    ...
                    <div class="main-controls-col">
                        ...
                        <div class="main-controls-buttons">
                           ...
                           <button data-action="activate-profile" id="activate-profile-btn" class="activate-btn" ${!isGm ? "disabled" : ""
                ...
                <div class="main-controls-buttons" style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                   ...
                   <button data-action="new-clean-profile" id="new-clean-profile-btn" title="Create a new appearance profile based on the module's default settings. (GM Only)" ${!isGm ? "disabled" : ""
				}>
                       <i class="fas fa-plus-square"></i> New Clean Profile
                   </button>
                   <button data-action="copy-settings" id="copy-active-settings-btn" title="Copy the current active settings to the clipboard, which can then be used to create a new hard-coded profile. (GM Only)" ${!isGm ? "disabled" : ""
				}>
			...
			contentHTML = `
                <div class="main-controls-init">
                    ...
                    <button data-action="create-scene-profiles" ${!isGm ? "disabled" : ""
				}><i class="fas fa-unlock-alt"></i> Create First Profile</button>
                    <button data-action="new-clean-profile" style="margin-top: 5px;" ${!isGm ? "disabled" : ""
...
//
class SimpleUIPanel extends Application {
	constructor(options = {}) {
		super(options);
	...
	_buildHTML() {
		...
		for (const [key, data] of Object.entries(CLIENT_OVERRIDES_CONFIG)) {
			...
			controlsHTML += `
                            <div class="simple-control-row" title="${data.tooltip
				}">
                                <label for="simple-enabled-${key}">${data.name
				}</label>
                                <div class="simple-widgets">
                                    ${intensitySlider}
                                    <input type="checkbox" id="simple-enabled-${key}" data-key="${key}" data-type="enabled" ${isEnabled ? "checked" : ""
				}>
		...
		return `
                    <style>
                        ...
                        .simple-ui-footer { margin-top: 10px; padding-top: 10px; border-top: 1px solid #555; text-align: center; }
...
class UserGuide extends Application {
	constructor(options = {}) {
		super(options);
	}
