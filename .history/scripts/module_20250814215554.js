    ...
    "sceneTransition": {
        ...
        "randomHints": [
            ...
            "You can add more hints in the Map Shine editor under Screen Effects > Scene Transition Effect."
        ]
        },
        "pauseEffect": {
        "enabled": true,
        ...
        "lut": {
                    ...
                    "diagnosticMode": 0,
                    "diagnosticSlice": 0.0,
                    "domainMin": { "r": 0.0, "g": 0.0, "b": 0.0 },
                    "domainMax": { "r": 1.0, "g": 1.0, "b": 1.0 },
                    "preLutBlur": {
                        "enabled": true,
                        "amount": 1.0
                    },
        ...
        "enabled": true,
        "blendMode": 1,
        "particleTexture": "modules/map-shine/assets/spark.webp",
        "frequency": 0.2,
        "lifetime": {
            "min": 1.5,
            "max": 3
...
function parseCubeLut(str) {
    ...
    class MapShineLifecycle {
        ...
        static finalizeConfigurationAndUI() {
            ...
            const EFFECT_TEXTURE_MAP = {
                ...
                fire: 'fire',
                fireSparks: 'fire', // ADDED FIRE SPARKS TO THE MAP
            };
    ...
    class SceneChangeManager {
        ...
        async _performTeardown(tornDownCanvas) {
            ...
            // Reset global managers to a clean state
        ...
        async _performSetup(canvas) {
            ...
            // Based on the dependency check, run the appropriate setup.
            if (dependenciesReady) {
                // The Illumination Buffer is ready, so we can run the full setup that uses it.
                await MapShineLifecycle.beginPersistentDiscovery(canvas);
            } else {
                // If dependencies are not ready, we cannot use light-aware features.
                // We still need to discover textures for other effects.
    ...
    const PARTICLE_EFFECT_DEFINITIONS = {
        ...
        fireSparks: {
            title: "Fire Sparks",
            description: "Creates sparks that fly off the fire in turbulent paths. Requires a _Fire.webp map.",
            configPath: 'fire.sparks',
            triggerTexture: 'fire',
            buildEmitterConfig: (effectConfig, targetData) => buildSparkEmitterConfig(effectConfig, targetData, 'fire')
        }
    };
    ...
    class ParticleEffectController {
        ...
        static getSettingsHTML(effectKey) {
            ...
            if (effectKey === 'fire') {
                content += `
                    ...
                    `;
                // Manually add the HTML for the sparks effect here
                content += `
                        <div id="new-controls-column-0">
                            <details id="details-fire-sparks" open>
                                <summary><span class="accordion-toggle"></span>
                                    <div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('fire.sparks.enabled', 'Sparks & Embers', true)}</div>
                                </summary>
                                <div style="padding-left: 15px;">
                                    <p class="description-text">Adds sparks that fly upwards in turbulent paths.</p>
                                    ${DebuggerUIBuilder._createSelectHTML('fire.sparks.blendMode', 'Blend Mode', BLEND_MODE_OPTIONS)}
                                    ${DebuggerUIBuilder._createTextInputHTML('fire.sparks.particleTexture', 'Particle Texture')}
                                    <details><summary><span class="accordion-toggle"></span><strong>Spawning</strong></summary><div style="padding-left:15px;">
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.frequency', 'Spawn Rate (s)', 0.01, 2, 0.01)}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.lifetime.min', 'Min Lifetime (s)', 0.5, 5, 0.1)}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.lifetime.max', 'Max Lifetime (s)', 0.5, 5, 0.1)}
                                    </div></details>
                                    <details><summary><span class="accordion-toggle"></span><strong>Appearance</strong></summary><div style="padding-left:15px;">
                                        ${DebuggerUIBuilder._createColorPickerHTML('fire.sparks.color.start', 'Start Color')}
                                        ${DebuggerUIBuilder._createColorPickerHTML('fire.sparks.color.end', 'End Color')}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.alpha.max', 'Max Alpha', 0, 1, 0.01)}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.scale.sizeMultiplier', 'Global Size', 0.1, 2, 0.05)}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.scale.start', 'Start Scale', 0.1, 2, 0.05)}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.scale.end', 'End Scale', 0, 2, 0.05)}
                                        ${DebuggerUIBuilder._createSliderHTML('fire.sparks.scale.minMult', 'Random Size Min', 0.1, 1, 0.01)}
                                    </div></details>
                                    <details open><summary><span class="accordion-toggle"></span><strong>Path & Movement</strong></summary><div style="padding-left:15px;">
                                        <details><summary><span class="accordion-toggle"></span><strong>Speed Along Path</strong></summary><div style="padding-left:15px;">
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.speed.start', 'Start Speed', 10, 200, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.speed.end', 'End Speed', 10, 200, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.speed.minMult', 'Random Speed Min', 0.1, 1, 0.01)}
                                        </div></details>
                                        <details open><summary><span class="accordion-toggle"></span><strong>Path Shape</strong></summary><div style="padding-left:15px;">
                                            <p class="description-text">Controls the random sine wave path for each spark.</p>
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.amplitude.min', 'Min Wave Width', 0, 100, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.amplitude.max', 'Max Wave Width', 0, 100, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.frequency.min', 'Min Wave Freq', 10, 200, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.frequency.max', 'Max Wave Freq', 10, 200, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.damping', 'Damping', 0, 1, 0.05, 'How quickly the path straightens out over the spark\'s life.')}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.angle.min', 'Min Start Angle', -90, 90, 1)}
                                            ${DebuggerUIBuilder._createSliderHTML('fire.sparks.path.angle.max', 'Max Start Angle', -90, 90, 1)}
                                        </div></details>
                                    </div></details>
                                </div>
                            </details>
        ...
        async _createEmitterForTarget(targetData, targetId) {
            ...
            // Create a shallow copy of targetData to avoid modifying the original shared object.
            ...
            if (this.definition.configPath === 'dust' && localTargetData.dust && localTargetData.structural) {
                ...
                // Generate the mask using the original string paths from the copied data.
                ...
                if (customMaskTexture) {
                    // Modify the COPY, not the original shared object.
                    // The buildParticleEmitterConfig function will now receive the generated texture.
            ...
            try {
                ...
                // Pass the (potentially modified) local copy to the config builder.
                const emitterConfig = this.definition.buildEmitterConfig(currentEffectConfig, localTargetData);
    ...
    const buildSparkEmitterConfig = (effectConfig, targetData, maskKey) => {
        ...
        const spawnBehavior = {
            ...
            config: {
                ...
                data: {
                    ...
                    x: 0,
                    y: 0,
                    threshold: 230, // Sparks come from the hottest parts
        ...
        const behaviors = [{
            ...
            spawnBehavior,
            {
                ...
                config: {
                    alpha: {
                        list: [{ value: 0, time: 0 }, { value: config.alpha?.max ?? 1.0, time: config.alpha?.fadeIn ?? 0.0 }, { value: config.alpha?.max ?? 1.0, time: 1.0 - (config.alpha?.fadeOut ?? 1.0) }, { value: 0, time: 1 }]
                    }
                }
            },
            {
                ...
                config: {
                    scale: {
                        list: [{ value: (config.scale?.start ?? 1.0) * (config.scale?.sizeMultiplier ?? 1.0), time: 0 }, { value: (config.scale?.end ?? 0.1) * (config.scale?.sizeMultiplier ?? 1.0), time: 1 }]
                    },
                    minMult: config.scale?.minMult ?? 0.5
                }
            },
            {
                type: 'color',
                config: {
                    color: {
                        list: [{ value: config.color?.start ?? "#FFFFFF", time: 0 }, { value: config.color?.end ?? "#FFFFFF", time: 1 }]
                    }
                }
            },
            // This is the new custom behavior for sparks
            {
                type: 'sparkPath',
                config: config.path
            }
        ...
        return {
            ...
            emitterLifetime: -1,
            maxParticles: Math.max(1, 2000 * 0.5), // Sparks are less dense than fire
            pos: {
    ...
    /**
     * A custom particle behavior that moves particles along a unique, randomized sine-wave path.
     * This is used to create a turbulent "spark" effect.
     */
    class SparkPathBehavior {
        ...
        constructor(config) {
            ...
            // Create the property list for speed over the particle's lifetime
            this.speedList = new PIXI.particles.PropertyList(false);
            this.speedList.reset(PIXI.particles.PropertyNode.createList(config.speed));
        }
        ...
        initParticles(first) {
            ...
            while (next) {
                ...
                const pConfig = next.config;

                // Store initial rotation (with randomization) and position
                ...
                // Generate and store unique path parameters for this particle
                ...
                // Randomize speed multiplier
                pConfig.speedMult = this._getRandom(config.speed.minMult, 1);
        ...
        updateParticle(particle, deltaSec) {
            ...
            // Increase linear movement based on the particle's speed over its lifetime
            const speed = this.speedList.interpolate(particle.agePercent) * pConfig.speedMult;
            pConfig.movement += speed * deltaSec;

            // Calculate the current y-position on the sine-wave path
            const x = pConfig.movement;
            // Apply damping to the amplitude, causing the path to straighten over time
            const amplitude = pConfig.pathAmplitude * (1.0 - (pConfig.pathDamping * particle.agePercent));
            const y = amplitude * Math.sin((x / pConfig.pathFrequency) + pConfig.pathOffset);

            const helperPoint = new PIXI.Point(x, y);

            // Rotate the calculated point by the particle's unique initial rotation
            ...
            // Set the final particle position relative to its spawn point
...
class ColorCorrectionFilter extends PIXI.Filter {
    ...
    class LutUtils {
        ...
        static async getLutData(path) {
            ...
            if (path.toLowerCase().endsWith('.cube')) {
                result = await this._getLutDataFromCube(path);
            } else {
                result = await this._getLutDataFromImage(path);
            }
        ...
        static async _getLutDataFromImage(path) {
            try {
                ...
                for (const size of commonSizes) {
                    ...
                    if (texture.width === slicesPerRow * size && texture.height === numRows * size) {
                        console.log(`Map Shine | LUT texture loaded: ${path} (Detected Size: ${size}, Grid: ${slicesPerRow}x${numRows})`);
                        return { texture, size, slicesPerRow, numRows, domain: [[0,0,0], [1,1,1]] }; // Return default domain for images
                    }
                }
                 throw new Error(`Could not determine LUT size from image dimensions (${texture.width}x${texture.height}).`);
            } catch (error) {
                console.error(`Map Shine | Failed to load LUT from image path: ${path}`, error);
                ui.notifications.error(`Map Shine | ${error.message}`);
                return null;
            }
        }
    
        static async _getLutDataFromCube(path) {
            try {
                ...
                const domain = parsed.domain; // Get the domain from the parsed data
    
                const slicesPerRow = Math.ceil(Math.sqrt(size));
                const numRows = Math.ceil(size / slicesPerRow);
                const canvas = document.createElement('canvas');
                canvas.width = size * slicesPerRow;
                canvas.height = size * numRows;
                ...
                for (let b = 0; b < size; b++) {
                    for (let g = 0; g < size; g++) {
                        for (let r = 0; r < size; r++) {
                            const gridX = b % slicesPerRow;
                            const gridY = Math.floor(b / slicesPerRow);
                            const finalX = (gridX * size) + r;
                            const finalY = (gridY * size) + g;
                            
                            const pixelIndex = (finalY * canvas.width + finalX) * 4;
                            const lutIndex = (b * size * size + g * size + r) * 3;
    
                            imageData.data[pixelIndex]     = data[lutIndex] * 255;
                            imageData.data[pixelIndex + 1] = data[lutIndex + 1] * 255;
                            imageData.data[pixelIndex + 2] = data[lutIndex + 2] * 255;
                ...
                console.log(`Map Shine | LUT .cube file loaded and processed: ${path} (Size: ${size})`);
                return { texture, size, slicesPerRow, numRows, domain }; // Return the domain
            } catch (error) {
                console.error(`Map Shine | Failed to load or parse .cube LUT from path: ${path}`, error);
                ui.notifications.error(`Map Shine | Failed to process .cube LUT. See console.`);
                return null;
            }
        }
    }
    ...
    class LutFilter extends PIXI.Filter {
        constructor() {
            const fragment = `
                ...
                // LUT layout
                uniform float uLutSize;        // e.g. 32
                uniform float uSlicesPerRow;   // e.g. ceil(sqrt(size))
                uniform float uNumRows;        // e.g. ceil(size / slicesPerRow)
    
                // simple on/off and strength
                uniform float uIntensity;
                ...
                void main() {
                    ...
                    // If disabled or no LUT, passthrough
                    if (uIntensity <= 0.0 || uLutSize <= 1.0) {
                        gl_FragColor = src;
                        return;
                    }
    
                    // Work in unpremultiplied space
                    float a = src.a;
                    vec3 color = src.rgb;
                    if (a > EPS) color = color / a;
    
                    // Clamp to safe range
                    color = clamp(color, 0.0, 1.0);
    
                    // Compute closest integer indices in LUT (nearest sampling)
                    float maxIndex = max(uLutSize - 1.0, 1.0);
                    vec3 idx = floor(color * maxIndex + 0.5); // round to nearest index
    
                    // Separate indices
                    float rIdx = clamp(idx.r, 0.0, maxIndex);
                    float gIdx = clamp(idx.g, 0.0, maxIndex);
                    float bIdx = clamp(idx.b, 0.0, maxIndex);
    
                    // Determine which blue slice (b) we use
                    float sliceIndex = bIdx;
                    float sliceX = mod(sliceIndex, uSlicesPerRow);
                    float sliceY = floor(sliceIndex / uSlicesPerRow);
    
                    // Offsets and sizes (normalized)
                    vec2 sliceOffset = vec2(sliceX, sliceY) / vec2(uSlicesPerRow, uNumRows);
                    vec2 sliceSize = vec2(1.0) / vec2(uSlicesPerRow, uNumRows);
    
                    // Position inside slice: use texel centers (idx + 0.5)
                    vec2 uvInSlice = (vec2(rIdx, gIdx) + 0.5) / uLutSize;
    
                    // Final UV in LUT texture
                    vec2 uv = sliceOffset + uvInSlice * sliceSize;
    
                    // Sample LUT (texture must be NEAREST and not premultiplied)
                    vec3 lutSample = texture2D(uLutTexture, uv).rgb;
    
                    // Result assumed to be same color space as input (neutral LUT)
                    vec3 result = mix(color, lutSample, uIntensity);
    
                    // Re-premultiply for output
                    if (a > EPS) result *= a;
    
                    // Clamp final output just in case
            ...
            super(PIXI.Filter.defaultVertexSrc, fragment, {
                uLutTexture: PIXI.Texture.EMPTY,
                uLutSize: 1.0,
                uSlicesPerRow: 1.0,
                uNumRows: 1.0,
                uIntensity: 1.0
            ...
            this._lutTexturePath = null;
        }
    
        // Apply LUT data (texture must already be a PIXI.Texture)
        applyLutData({ texture, size, slicesPerRow, numRows }) {
            if (!this.uniforms) return;
    
            // If we have a canvas/image texture, force exact texel sampling and no premultiplication.
            if (texture && texture.baseTexture) {
                try {
                    texture.baseTexture.scaleMode = PIXI.SCALE_MODES.NEAREST; // exact texel
                    texture.baseTexture.wrapMode = PIXI.WRAP_MODES.CLAMP;
                    // ensure the base texture won't be treated as premultiplied
                    if (typeof texture.baseTexture.premultiplyAlpha !== "undefined") {
                        texture.baseTexture.premultiplyAlpha = false;
                    }
                    // Ensure upload / rebind
                    texture.baseTexture.update();
                } catch (err) {
                    console.warn("LutFilter | Failed to set baseTexture flags:", err);
                }
            }
    
            this.uniforms.uLutTexture = texture || PIXI.Texture.EMPTY;
            this.uniforms.uLutSize = size || 1.0;
            this.uniforms.uSlicesPerRow = slicesPerRow || 1.0;
            this.uniforms.uNumRows = numRows || 1.0;
        }
    
        // Generate a neutral LUT canvas (grid layout)
        static _generateNeutralLutCanvas(size = 32) {
            const slicesPerRow = Math.ceil(Math.sqrt(size));
            const numRows = Math.ceil(size / slicesPerRow);
            const width = size * slicesPerRow;
            const height = size * numRows;
            ...
            for (let b = 0; b < size; b++) {
                const sliceX = b % slicesPerRow;
                const sliceY = Math.floor(b / slicesPerRow);
                for (let g = 0; g < size; g++) {
                    for (let r = 0; r < size; r++) {
                        const x = sliceX * size + r;
                        const y = sliceY * size + g;
                        const idx = (y * width + x) * 4;
    
                        const rVal = size > 1 ? r / (size - 1) : 0;
                        const gVal = size > 1 ? g / (size - 1) : 0;
                        const bVal = size > 1 ? b / (size - 1) : 0;
        ...
        // Generate a neutral LUT PNG and trigger a download for use as a template
        static generateAndDownloadNeutralLut(size = 32) {
            const canvas = LutFilter._generateNeutralLutCanvas(size);
            if (!canvas) {
                console.error("LutFilter | Failed to generate LUT canvas for download");
                ui.notifications?.error("Failed to generate LUT canvas");
                return;
            }
            try {
                const link = document.createElement('a');
                link.download = `neutral-lut-${size}x${size}x${size}.png`;
                link.href = canvas.toDataURL('image/png');
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                ui.notifications?.info(`Neutral LUT PNG downloaded (${size}x${size}x${size}).`);
            } catch (e) {
                console.error("LutFilter | Failed to download neutral LUT PNG", e);
                ui.notifications?.error("Failed to download neutral LUT PNG. See console.");
            }
        }
    
        // Generate neutral LUT and apply to this filter instance
        async generateAndApplyNeutralLut(size = 32) {
            const canvas = LutFilter._generateNeutralLutCanvas(size);
            if (!canvas) {
                console.error("LutFilter | Failed to generate LUT canvas");
                ui.notifications?.error("Failed to generate LUT canvas");
                return;
            }

            // Create PIXI texture with NEAREST and NO premultiplication
            const base = new PIXI.BaseTexture(canvas, {
                scaleMode: PIXI.SCALE_MODES.NEAREST,
                wrapMode: PIXI.WRAP_MODES.CLAMP,
                // attempt to prevent premultiplying at upload
                premultiplyAlpha: false
            });
            // In some PIXI builds there's alphaMode; set if present
            if (typeof base.alphaMode !== "undefined") {
                try { base.alphaMode = PIXI.ALPHA_MODES.NO_PREMULTIPLIED_ALPHA; } catch (e) { /* ignore */ }
            }
            base.update();
            ...
            // remember we applied an in-memory LUT
            this._lutTexturePath = `neutral-in-memory-${Date.now()}`;
            console.log("LutFilter | Neutral LUT applied (in-memory)", { size, slicesPerRow, numRows });
            ui.notifications?.info(`Neutral LUT applied (${size}x${size}x${size}).`);
        }
    
        // Convenience accessor for intensity
        get intensity() { return this.uniforms?.uIntensity ?? 1.0; }
        set intensity(v) { if (this.uniforms) this.uniforms.uIntensity = Math.max(0, Math.min(1, v)); }
    }
    ...
    class ScreenEffectsManager {
        ...
        static getSettingsHTML() {
            const DIAGNOSTIC_MODES = {
                "Normal Operation": 0,
                "Show Processed Input": 2,
                "Show Lookup UVs": 3,
                "Show Blue Slice": 4,
                "Show Neutral LUT Error": 5,
                "Scan LUT Slice": 6,
                "Show Input Clipping": 7,
                "Show Out-of-Range Input": 8,
                "Test: No Interpolation": 9,
                "Show Final Lookup UVs": 10
            };
    
            let content = DebuggerUIBuilder._createAccordionHTML('sceneTransition', 'Scene Transition Effect', `
            ...
            content += DebuggerUIBuilder._createAccordionHTML('pauseEffect', 'Pause Transition Effect', `
            ...
            content += DebuggerUIBuilder._createAccordionHTML('postProcessing', 'Post Processing', `
                    ...
                    <details id="details-postProcessing-lut"><summary><span class="accordion-toggle"></span><div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('postProcessing.lut.enabled', 'Color Grading (LUT)', true)}</div></summary>
                        <div>
                            <p class="description-text">Applies a color look-up table for advanced color grading. Select a preset or choose 'Custom Path' to provide your own.</p>
                            ${DebuggerUIBuilder._createPresetSelectHTML('postProcessing.lut.presetName', 'Preset', LUT_PRESETS)}
                            <div id="lut-custom-path-wrapper" style="display: none;">
                                ${DebuggerUIBuilder._createTextInputWithPickerHTML('postProcessing.lut.texturePath', 'Custom Path', 'Path to your LUT file (.png, .webp)', 'any')}
                            </div>
                            ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.intensity', 'Intensity', 0, 1, 0.01)}
                            <button id="generate-neutral-lut-btn" title="Generate and download a neutral 32x32 LUT file in the correct format. This can be used as a template for creating your own custom LUTs." style="width: 100%; margin-top: 5px; height: 24px;">Generate Neutral LUT Template</button>
                            <button id="apply-neutral-lut-btn" title="Instantly generate and apply a neutral LUT from memory. This is the fastest way to test if the LUT system is working correctly." style="width: 100%; margin-top: 5px; height: 24px;">Generate & Apply Neutral LUT</button>
                            
                            <details id="details-postProcessing-lut-input-processing" style="border-color: #4a4; margin-top: 5px;">
                                <summary style="font-weight: normal; color: #8d8;"><div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('postProcessing.lut.inputProcessing.enabled', 'Input Processing (Pre-LUT)', true)}</div></summary>
                                <p class="description-text">Adjusts the image *before* the LUT is applied. Useful for fixing issues or for creative grading.</p>
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.inputProcessing.brightness', 'Brightness', -1, 1, 0.01)}
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.inputProcessing.contrast', 'Contrast', 0, 3, 0.01)}
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.inputProcessing.saturation', 'Saturation', 0, 3, 0.01)}
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.inputProcessing.gamma', 'Gamma', 0.2, 2.5, 0.01)}
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.inputProcessing.hue', 'Hue Shift', -0.5, 0.5, 0.005)}
                            </details>

                            <details id="details-postProcessing-lut-domain" style="border-color: #4a4; margin-top: 5px;">
                                <summary style="font-weight: normal; color: #8d8;"><strong>Domain Remapping (Input Scaling)</strong></summary>
                                <p class="description-text">Adjust the color range of the scene *before* it is sent to the LUT. This can fix artifacts from posterization or color space mismatches.</p>
                                <details><summary><span class="accordion-toggle"></span><strong>Domain Min (Black Point)</strong></summary><div style="padding-left:15px;">
                                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.domainMin.r', 'Red', 0, 1, 0.001)}
                                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.domainMin.g', 'Green', 0, 1, 0.001)}
                                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.domainMin.b', 'Blue', 0, 1, 0.001)}
                                </div></details>
                                <details><summary><span class="accordion-toggle"></span><strong>Domain Max (White Point)</strong></summary><div style="padding-left:15px;">
                                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.domainMax.r', 'Red', 0, 1, 0.001)}
                                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.domainMax.g', 'Green', 0, 1, 0.001)}
                                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.domainMax.b', 'Blue', 0, 1, 0.001)}
                                </div></details>
                            </details>

                            <details id="details-postProcessing-lut-preblur" style="border-color: #4a4; margin-top: 5px;">
                                <summary style="font-weight: normal; color: #8d8;">
                                    <div class="summary-control">
                                        ${DebuggerUIBuilder._createCheckboxHTML('postProcessing.lut.preLutBlur.enabled', 'Input Smoothing (Pre-Blur)', true)}
                                    </div>
                                </summary>
                                <p class="description-text">Blurs the scene slightly *before* the LUT is applied. This can eliminate noise and banding artifacts by smoothing out problematic color values.</p>
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.preLutBlur.amount', 'Blur Radius (pixels)', 0, 5, 1)}
                            </details>

                            <details id="details-postProcessing-lut-diagnostics" style="border-color: #aa4; margin-top: 5px;"><summary style="font-weight: normal; color: #dd8;"><strong>Shader Diagnostics</strong></summary>
                                ${DebuggerUIBuilder._createSelectHTML('postProcessing.lut.diagnosticMode', 'Mode', DIAGNOSTIC_MODES)}
                                ${DebuggerUIBuilder._createSliderHTML('postProcessing.lut.diagnosticSlice', 'Slice to Scan', 0, 1, 0.01, 'Used with "Scan LUT Slice" mode.')}
                            </details>
                        </div>
                    </details>
        ...
        static _updateContainerFilters() {
            ...
            const filterClasses = [WaterEffectsFilter, PrismFilter, HeatDistortionFilter, VignetteFilter, LensDistortionFilter, ChromaticAberrationFilter, ColorCorrectionFilter, LutFilter];
        ...
        static setupAllGlobalFilters() {
            ...
            try {
                const lutFilter = new LutFilter();
                this.addFilter('lut', lutFilter);
            } catch (e) {
                ppErrors.push('Lut');
            }
        ...
        static updateAllFiltersFromConfig(config) {
            ...
            const lutFilter = this.getFilter('lut');
            if (lutFilter instanceof LutFilter) {
                const lutConfig = pp.lut;
                const hasPath = !!lutConfig.texturePath && !lutConfig.texturePath.startsWith('neutral-lut-in-memory-');
                
                lutFilter.enabled = config.enabled && pp.enabled && lutConfig.enabled && !!lutConfig.texturePath;
                lutFilter.intensity = lutConfig.intensity;
                
                const u = lutFilter.uniforms;
                u.uDiagnosticMode = lutConfig.diagnosticMode ?? 0;
                u.uDiagnosticSlice = lutConfig.diagnosticSlice ?? 0.0;
    
                const blurConfig = lutConfig.preLutBlur || { enabled: false, amount: 0.0 };
                u.uPreLutBlurEnabled = blurConfig.enabled;
                u.uPreLutBlurAmount = blurConfig.amount;
                
                const ipConfig = lutConfig.inputProcessing || {};
                u.uIpEnabled = ipConfig.enabled;
                u.uIpSaturation = ipConfig.saturation;
                u.uIpBrightness = ipConfig.brightness;
                u.uIpContrast = ipConfig.contrast;
                u.uIpGamma = ipConfig.gamma;
                u.uIpHue = ipConfig.hue;
                ...
                if (lutConfig.domainMin) {
                    u.uDomainMin = [lutConfig.domainMin.r, lutConfig.domainMin.g, lutConfig.domainMin.b];
                }
                if (lutConfig.domainMax) {
                    u.uDomainMax = [lutConfig.domainMax.r, lutConfig.domainMax.g, lutConfig.domainMax.b];
                }
    
                if (hasPath && lutFilter._lutTexturePath !== lutConfig.texturePath) {
                    lutFilter._lutTexturePath = lutConfig.texturePath; 
                    LutUtils.getLutData(lutConfig.texturePath).then(lutData => {
                        if (lutData && lutFilter) {
                            lutFilter.applyLutData(lutData);
                            if (game.mapShine.profileManager) {
                                game.mapShine.profileManager.recordUserChange('postProcessing.lut.domainMin', { r: lutData.domain[0][0], g: lutData.domain[1][0], b: lutData.domain[2][0] });
                                game.mapShine.profileManager.recordUserChange('postProcessing.lut.domainMax', { r: lutData.domain[0][1], g: lutData.domain[1][1], b: lutData.domain[2][1] });
                                if (game.mapShine.debugger) {
                                    game.mapShine.debugger.eventHandler.updateAllControls();
                                }
                            }
                        } else if (lutFilter) {
                            lutFilter.applyLutData({ texture: PIXI.Texture.EMPTY, size: 1.0, domain: [[0,0,0], [1,1,1]] });
                        }
                    });
                } else if (!lutConfig.texturePath) {
                    lutFilter.applyLutData({ texture: PIXI.Texture.EMPTY, size: 1.0 });
                    lutFilter._lutTexturePath = null;
                }
            }
    ...
    class DebuggerUIBuilder {
        ...
        static _getStyles() {
            return `<style>
                        ...
                        #material-editor-debugger .file-picker-btn {
                            ...
                            width: 22px;
                            height: 22px;
                            line-height: 18px; /* To center icon */
                            padding: 0;
                            font-size: 12px;
                            background: #3a3a3a;
                        ...
                        #material-editor-debugger .file-picker-btn:hover {
                            background: #555;
                            border-color: #888;
                        }
            ...
            return `
                <div id="${id}" class="list-manager-container" data-path="${path}">
                    ...
                    <div id="${listContainerId}" class="list-items-container" style="display: flex; flex-direction: column; gap: 4px; margin-top: 5px; padding-left: 10px;">
                        <!-- Hint items will be dynamically inserted here -->
                    </div>
                </div>
                <style>
        ...
        _getEffectSections() {
            return [
                ...
                ParticleEffectController.getSettingsHTML('fireSparks'), // New entry here
    ...
    class DebuggerEventHandler {
        ...
        initialize() {
            ...
            this._makeDraggable();
            // Asynchronously populate the LUT dropdown, then update all controls
            this._populateLutDropdown().then(() => {
                this.updateAllControls();
            });
            // Update controls immediately for a responsive UI
            this.updateAllControls();
        }
        ...
        addEventListeners() {
            ...
            // Delegated listener for click events
        ...
        async _populateLutDropdown() {
            ...
            // Start with the hardcoded presets
                ...
                for (const filePath of dirContents.files) {
                    if (filePath.toLowerCase().endsWith('.cube')) {
                        ...
                        const friendlyName = filename
                            .replace(/\.cube$/i, '')    // Remove extension
            ...
            // Save current value to reselect it later
            const currentValue = dropdown.value;
        
            // Clear existing options
            dropdown.innerHTML = '';
        
            // Populate with new combined list
            for (const [key, data] of Object.entries(this.allLutPresets)) {
                const option = new Option(data.name, key);
                dropdown.add(option);
            }
            
            // Try to reselect the previous value, or a sensible default
        ...
        async _handleGenericInput(e) {
            ...
            if (path === 'postProcessing.lut.presetName') {
                ...
                this.updateAllControls(); // Refresh UI with new domain values
                this._updateLutControlVisibility();
            }
        ...
        _onGenerateNeutralLut() {
            LutFilter.generateAndDownloadNeutralLut();
        }
    
        _onApplyNeutralLut() {
            const lutFilter = ScreenEffectsManager.getFilter('lut');
            if (lutFilter?.generateAndApplyNeutralLut) {
                lutFilter.generateAndApplyNeutralLut();
            } else {
                ui.notifications.error("Could not find the live LUT filter instance.");
            }
        }
    }
    ...
    Hooks.once('init', () => {
        ...
        // The old direct hooks are removed. The manager handles this now.
    });