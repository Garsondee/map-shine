...
class MapShineLifecycle {
    ...
    static async runFullSetup(canvas) {
        ...
        // The final initialization is now handled by the polling mechanism in ParticleLayer._onAnimate.
        // We no longer use a one-time hook here.
...
class ParticleLayer extends CanvasLayer {
    ...
    _onAnimate(deltaTime) {
        ...
        if (!this._initialized && game.mapShine.systemsReady) {
            ...
            }, 2500); // 2.5 second delay which is proven to be stable.
        }
        // --- END INITIALIZATION ---
                    ...
                    <ul class="mp-points-list">
                        ${selectedGroup.points.map((p, i) => `
                            <li class="mp-point-item">
                                ...
                                <button type="button" data-action="delete-point" data-point-index="${i}" title="Delete Point">Ãƒâ€”</button>
                            </li>
                        `).join('')}
                    </ul>
