...
"sceneTransition": {
    ...
    "colorCorrection": {
        ...
        "curves": {
            "enabled": false,
            "activeChannel": "rgb",
            "rgb":   { "points": [{ "x": 0, "y": 0 }, { "x": 0.25, "y": 0.25 }, { "x": 0.75, "y": 0.75 }, { "x": 1, "y": 1 }] },
            "red":   { "points": [{ "x": 0, "y": 0 }, { "x": 0.25, "y": 0.25 }, { "x": 0.75, "y": 0.75 }, { "x": 1, "y": 1 }] },
            "green": { "points": [{ "x": 0, "y": 0 }, { "x": 0.25, "y": 0.25 }, { "x": 0.75, "y": 0.75 }, { "x": 1, "y": 1 }] },
            "blue":  { "points": [{ "x": 0, "y": 0 }, { "x": 0.25, "y": 0.25 }, { "x": 0.75, "y": 0.75 }, { "x": 1, "y": 1 }] }
        }
    },
    "vignette": {
        "enabled": true,
...
class DebuggerEventHandler {
    ...
    initialize() {
        ...
        this.updateAllControls();
        // Initialize the color favorites list
    ...
    _initializeCurveEditor() {
        ...
        if (curveEditorContainer) {
            ...
            // Add listeners for the new channel radio buttons
            ...
            // Initialize the editor with the active channel's data from the config
    ...
    async _handleGenericInput(e) {
        ...
        if (e.target.type === 'radio') {
            if (!e.target.checked) return; // Only process the selected radio button
        }
    ...
    async _onCurveChange(points, options = {}) {
        ...
        if (activeChannel === 'rgb') {
            // If editing the master RGB curve, update all channels simultaneously.
            ...
            // If editing an individual channel, only update that specific channel.
        ...
        // Trigger a full update of all visual systems to apply the new LUT.
    ...
    _onCurveChannelChange(e) {
        ...
        // The recordUserChange is already handled by the generic input handler
        // so we just need to update the editor's view.
        this._updateCurveEditorView();
    }
    ...
    _updateCurveEditorView() {
        ...
        // Update curve color for visual feedback
        const colorMap = {
            rgb: '#00aaff',
            red: '#ff6b6b',
            green: '#6bff6b',
    ...
    async _onApplyColorPreset() {
        ...
        // Record all changes through the profile manager to update the actual filters
        ...
        // Apply the curves configuration from the preset
        if (preset.curves) {
            await this.profileManager.recordUserChange('postProcessing.colorCorrection.curves', preset.curves);
            this._updateCurveEditorView();
        }

        // After recording the changes, force all visual systems to update with the new config.
        await this.profileManager.updateAllSystemsFromConfig();
    ...
    _onSaveColorFavorite() {
        ...
        // Get existing favorites from settings
        let favorites = [];
        try {
    ...
    async _applyColorFavorite(index) {
        ...
        // Apply favorite values to the config
        const ccConfig = this.config.postProcessing.colorCorrection;
