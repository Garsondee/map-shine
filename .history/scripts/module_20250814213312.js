...
class ColorCorrectionFilter extends PIXI.Filter {
    ...
    class LutFilter extends PIXI.Filter {
        ...
        applyLutData({ texture, size, slicesPerRow, numRows }) {
            ...
            if (texture && texture.baseTexture) {
                try {
                    ...
                    console.warn("MinimalLutFilter | Failed to set baseTexture flags:", err);
                }
            }
        ...
        static _generateNeutralLutCanvas(size = 32) {
            ...
            ctx.putImageData(imageData, 0, 0);
            return { canvas, size, slicesPerRow, numRows, width, height };
        }
    
        // Generate neutral LUT and apply to this filter instance
        async generateAndApplyNeutralLut(size = 32) {
            const lut = MinimalLutFilter._generateNeutralLutCanvas(size);
            if (!lut) {
                ui.notifications?.error("MinimalLutFilter: failed to generate LUT canvas");
                return;
            }
    
            // Create PIXI texture with NEAREST and NO premultiplication
            const base = new PIXI.BaseTexture(lut.canvas, {
                scaleMode: PIXI.SCALE_MODES.NEAREST,
                wrapMode: PIXI.WRAP_MODES.CLAMP,
            ...
            this.applyLutData({
                texture: tex,
                size: lut.size,
                slicesPerRow: lut.slicesPerRow,
                numRows: lut.numRows
            });
    
            // remember we applied an in-memory LUT
            this._lutTexturePath = `neutral-in-memory-${Date.now()}`;
            console.log("MinimalLutFilter | Neutral LUT applied (in-memory)", { size: lut.size, slicesPerRow: lut.slicesPerRow, numRows: lut.numRows });
            ui.notifications?.info(`MinimalLutFilter: neutral LUT applied (${size}³).`);
        }
    ...
    class DebuggerEventHandler {
        ...
        _onApplyNeutralLut() {
            ...
            if (lutFilter) {
                LutFilter.generateAndApplyNeutralLut(lutFilter);
            } else {
