...
class DynamicExposureManager {
    constructor() {
        ...
        this.lastCheckedLuminance = -1; // Luminance at the last interval check
        this.currentExposureBoost = 0;
        this.currentBlur = 0;
    ...
    initialize() {
        ...
        ScreenEffectsManager.addFilter('dynamicExposureBlur', this.blurFilter);
        ...
        Hooks.on('mapShine:activeTokenChanged', this._boundReset);
        window.addEventListener('resize', this._boundResize);

        this.isInitialized = true;
        console.log("Map Shine | DynamicExposureManager initialized with Fixed Interval Check method.");
    }
    ...
    reset() {
        this.lastCheckTime = 0;
        this.lastCheckedLuminance = -1;
        if (this.animation) {
            this.animation.kill();
            this.animation = null;
        }
        this._applyEffects(0, 0);
        this.debugData.strength = 0;
        this.debugData.delta = 0;
    }

    update(deltaTimeInSeconds) {
        if (!this.isInitialized || !this.ccFilter || !this.blurFilter || !this._preEffectTexture) return;

        this.config = game.mapShine.profileManager.activeConfig.postProcessing.dynamicExposure;
        if (!this.config?.enabled) {
            if (this.blurFilter.enabled || this.ccFilter.uniforms.uDynamicExposureBoost !== 0) {
                this._applyEffects(0, 0);
            }
            this.debugData.activeTokenName = "None (Effect Disabled)";
            return;
        }
        ...
        // --- TWO-PASS RENDER ---
        const originalExposure = this.ccFilter.uniforms.uDynamicExposureBoost;
        const originalBlurEnabled = this.blurFilter.enabled;
        this.ccFilter.uniforms.uDynamicExposureBoost = 0.0;
        this.blurFilter.enabled = false;

        this.renderer.render(canvas.stage, {
            renderTexture: this._preEffectTexture,
            clear: true
        });

        this.ccFilter.uniforms.uDynamicExposureBoost = originalExposure;
        this.blurFilter.enabled = originalBlurEnabled;
        ...
        this.debugData.luminance = currentLuminance;

        // --- FIXED INTERVAL CHECK LOGIC ---
        const now = performance.now();
        const timeSinceLastCheck = (now - this.lastCheckTime) / 1000.0;
        
        // Initialize the baseline on the very first run.
        if (this.lastCheckedLuminance < 0) {
            this.lastCheckedLuminance = currentLuminance;
            this.lastCheckTime = now;
            return;
        }

        // Only perform the check if the interval has passed.
        if (timeSinceLastCheck < this.config.checkInterval) {
            return;
        }

        const deltaLuminance = currentLuminance - this.lastCheckedLuminance;
        this.debugData.delta = deltaLuminance;

        if (deltaLuminance > this.config.triggerThreshold) {
            // The rate of change is the total change divided by the time since the last check.
            const changeRate = deltaLuminance / timeSinceLastCheck;
            const effectStrength = Math.min(1.0, changeRate / this.config.sensitivity);
            
            if (effectStrength > 0.1) {
                this.triggerEffect(effectStrength);
            }
        }
    ...
    triggerEffect(strength) {
        if (this.animation) {
            this.animation.kill();
        }

        const peakExposure = this.config.maxExposureBoost * strength;
        const peakBlur = this.config.maxBlur * strength;

        const animationState = {
            exposure: this.currentExposureBoost,
            blur: this.currentBlur,
            strength: this.debugData.strength
        };

        this.animation = gsap.timeline({
            onUpdate: () => {
                this.currentExposureBoost = animationState.exposure;
                this.currentBlur = animationState.blur;
                this.debugData.strength = animationState.strength;
                this._applyEffects(this.currentExposureBoost, this.currentBlur);
            },
            onComplete: () => {
                this.animation = null;
            }
        ...
        this.animation.to(animationState, {
        ...
        this.animation.to(animationState, {
            ...
            blur: 0,
            strength: 0,
            duration: this.config.decayDuration,
            ease: "power1.inOut"
        });
    }

    _applyEffects(exposure, blur) {
        if (!this.ccFilter || !this.blurFilter) return;

        this.ccFilter.uniforms.uDynamicExposureBoost = exposure;

        this.blurFilter.enabled = blur > 0.01;
    ...
    destroy() {
        ...
        this._applyEffects(0, 0);
        ScreenEffectsManager.removeFilter('dynamicExposureBlur');
        this.blurFilter = null;
        this.ccFilter = null;
        if (this._boundReset) {
            Hooks.off('mapShine:activeTokenChanged', this._boundReset);
        }
...
class ParticleEffectController {
    ...
    updateFromConfig(fullConfig) {
        ...
        // Determine visibility based on the config chain.
        ...
        // The emit state is now managed frame-by-frame by the ParticleManager.
        // We only manage alpha here for effects like glint fading in darkness.
    ...
    static getManagedEffectsHTML() {
        ...
        const postProcessingHTML = `
            ...
            <details id="details-postProcessing-dynamicExposure">
                <summary>
                    <span class="accordion-toggle"></span>
                    <div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('postProcessing.dynamicExposure.enabled', 'Dynamic Exposure', true)}</div>
                </summary>
                <div style="padding-left: 15px;">
                    <p class="description-text">Simulates eye adaptation by boosting exposure and adding blur when a token moves from a dark area to a bright one.</p>
                    <div class="warning-box" style="background-color: #554422; border-color: #ffaa66;">
                        <strong style="color: #ffddaa;">PERFORMANCE WARNING:</strong> This effect can be demanding as it samples the screen every frame for the active token.
                    </div>
                    ...
                    <details id="details-de-readout" open>
                        ...
                        <div style="padding: 5px; background: rgba(0,0,0,0.3); border-radius: 3px; font-family: monospace; font-size: 11px;">
                            ...
                            <div class="control-row"><label>Luminance:</label><span id="de-current-lum">0.000</span></div>
                            <div class="control-row"><label>Change (Delta):</label><span id="de-delta-lum">0.000</span></div>
                            <div class="control-row"><label>Effect Strength:</label><span id="de-strength">0.000</span></div>
                        </div>
                    </details>
                    
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.triggerThreshold', 'Trigger Threshold', 0.05, 1.0, 0.01, 'Minimum increase in brightness (0-1) to trigger the effect.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.sensitivity', 'Sensitivity', 0.1, 5.0, 0.1, 'How strongly the effect reacts to brightness changes. Higher means a smaller change is needed for a full-strength effect.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.checkInterval', 'Check Interval (s)', 0.1, 1.0, 0.05, 'How often to check for brightness changes.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.maxExposureBoost', 'Max Exposure Boost', 0, 2, 0.05, 'The maximum exposure value added at peak effect strength.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.maxBlur', 'Max Blur', 0, 20, 0.5, 'The maximum blur strength applied at peak effect strength.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.attackDuration', 'Attack Duration (s)', 0.1, 2.0, 0.05, 'How quickly the effect reaches its peak.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.dynamicExposure.decayDuration', 'Decay Duration (s)', 1.0, 30.0, 0.5, 'How long it takes for the effect to fade out.')}
                </div>
            </details>
...
class DiagnosticLayer extends foundry.canvas.layers.CanvasLayer {
    ...
    async _tearDown(options) {
        ...
        this.diagnosticContainer?.destroy({ children: true });
        this.overlayContainer?.destroy({ children: true });
        this.fullscreenSprite?.destroy();
    ...
    getAvailableDebugTextures() {
        ...
        const layerChecks = {
            'intermediate_metallicShinePattern': { class: MetallicShineLayer, method: 'getPatternTexture', name: 'Metallic Shine Pattern' },
            'intermediate_waterDisplacement': { class: WaterFXLayer, name: 'Water Displacement' }, // Special handling needed
            'intermediate_heatNoise': { class: HeatDistortionLayer, name: 'Heat Noise' }, // Special handling needed
            'intermediate_iridescenceNoise': { class: IridescenceLayer, name: 'Iridescence Noise' }, // Special handling needed
            'intermediate_canopyNoise': { class: CanopyLayer, name: 'Canopy Noise' }, // Special handling needed
            'intermediate_structuralNoise': { class: StructuralShadowsLayer, name: 'Structural Noise' } // Special handling needed
        };

        for (const [key, check] of Object.entries(layerChecks)) {
            const layer = canvas.layers.find(l => l instanceof check.class);
            if (layer) {
                if (key === 'intermediate_waterDisplacement' && layer.displacementTexture) textures.intermediates[key] = check.name;
                else if (key === 'intermediate_heatNoise' && layer.noiseManager) textures.intermediates[key] = check.name;
                else if (key === 'intermediate_iridescenceNoise' && layer.distortionNoiseManager) textures.intermediates[key] = check.name;
                else if (key === 'intermediate_canopyNoise' && layer.distortionNoiseManager) textures.intermediates[key] = check.name;
                else if (key === 'intermediate_structuralNoise' && layer.intensityNoiseManager) textures.intermediates[key] = check.name;
                else if (check.method && typeof layer[check.method] === 'function' && layer[check.method]()) {
                    textures.intermediates[key] = check.name;
                }
            }
        }
...
class ProfileManager {
    ...
    _getEffectiveConfig() {
        ...
        if (worldDefaultProfile) {
            const effectsToCheck = ['sceneTransition', 'pauseEffect'];
...
class DebuggerEventHandler {
    ...
    _populateDiagnosticDropdown() {
        ...
        const createOptGroup = (label, options, valuePrefix = '') => {
            const group = document.createElement('optgroup');
            group.label = label;
            for (const [value, text] of Object.entries(options)) {
                const option = new Option(text, valuePrefix + value);
        ...
        if (!foundry.utils.isEmpty(available.intermediates)) {
            dropdown.appendChild(createOptGroup("Intermediate Textures", available.intermediates, 'intermediate_'));
        }
        if (!foundry.utils.isEmpty(available.external)) {
            dropdown.appendChild(createOptGroup("External Buffers", available.external, 'external_'));
        }
    ...
    _updateDynamicExposureReadout() {
        ...
        const lumEl = this.element.querySelector('#de-current-lum');
        const deltaEl = this.element.querySelector('#de-delta-lum');
        const strengthEl = this.element.querySelector('#de-strength');

        if (nameEl) nameEl.textContent = data.activeTokenName;
        if (lumEl) lumEl.textContent = data.luminance.toFixed(3);
        if (deltaEl) {
            const delta = data.delta;
            deltaEl.textContent = `${delta >= 0 ? '+' : ''}${delta.toFixed(3)}`;
            if (delta > manager.config?.triggerThreshold) {
                deltaEl.style.color = '#87CEFA'; // Light Blue
            } else if (delta < -manager.config?.triggerThreshold) {
                deltaEl.style.color = '#F08080'; // Light Coral
            } else {
                deltaEl.style.color = '';
            }
        }
        if (strengthEl) {
            ...
            strengthEl.style.color = `rgba(135, 206, 250, ${strength})`;
        }
    }
...
class DebuggerUIBuilder {
    ...
    buildRootElement() {
        ...
        element.querySelector('#material-editor-diagnostic-section').innerHTML = this._buildDiagnosticSection();
