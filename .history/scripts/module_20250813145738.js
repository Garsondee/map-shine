...
class SceneChangeManager {
    ...
    async fadeIn(config) {
        return new Promise(resolve => {
            ...
            tl.to(this.transitionOverlay, {
                ...
                ease: "power2.out",
                onComplete: () => { if (this.transitionOverlay) this.transitionOverlay.style.pointerEvents = 'none'; }
            }, "<0.2");
        });
    }
    ...
    async _performSetup(canvas) {
        ...
        // The rest of the setup logic is now delegated back to MapShineLifecycle
...
class ScreenEffectsManager {
    ...
    static getSettingsHTML() {
        let content = DebuggerUIBuilder._createAccordionHTML('sceneTransition', 'Scene Transition Effect', `
            ...
            ${DebuggerUIBuilder._createTextInputHTML('sceneTransition.logoPath', 'Logo Image Path', 'Path to an image file (e.g., PNG, WEBP) to display in the center.')}
...
class AmbientMaskManager {
    ...
    update() {
        ...
        // --- DEFENSIVE CHECK ---
        // Ensure the texture is valid AND matches the current canvas dimensions to prevent using a stale texture during scene transitions.
        if (!illuminationTexture?.valid || illuminationTexture.width !== screen.width || illuminationTexture.height !== screen.height) {
            if (ambientLayer.mask) ambientLayer.mask = null;
            // Log this situation if it happens, as it indicates a race condition is being handled.
            if (game.mapShine.profileManager.activeConfig.debug) {
                 // console.log("AmbientMaskManager DEBUG | Stale or invalid illumination texture detected. Waiting for a valid one...");
            }
            return;
        }
...
class LightingEffectManager {
    ...
    update() {
        ...
        // --- DEFENSIVE CHECK ---
        // A single check at the top for the primary dependency.
        const isIlluminationReady = illuminationAPI && illuminationTexture?.valid && illuminationTexture.width === screen.width && illuminationTexture.height === screen.height;
...
class StructuralShadowsLayer extends MaskedEffectLayer {
    constructor() {
        ...
        // Properties for throttled logging
        this._lastLogTime = 0;
        this._logInterval = 5000; // 5 seconds

        console.log("StructuralShadowsLayer DEBUG | constructor: New layer instance created.");
    }
    ...
    async _draw(options) {
        ...
        console.log(`%cStructuralShadowsLayer DEBUG | _draw: STARTING DRAW for scene ${canvas.scene.id}`, 'color: green; font-weight: bold;');
        
        this._lastLogTime = 0; // Reset log timer
        this._needsOutdoorsMaskUpdate = true;
        this.blendMode = PIXI.BLEND_MODES.NORMAL;
        const renderer = canvas.app.renderer;
        const screen = renderer.screen;
        ...
        this.cleanStructuralLightMask = PIXI.RenderTexture.create({ width: screen.width, height: screen.height });
        ...
        console.log(`%cStructuralShadowsLayer DEBUG | _draw: FINISHED DRAW`, 'color: green; font-weight: bold;');
    }
    ...
    _onAnimate(deltaTime) {
        ...
        const shouldLog = currentTime - this._lastLogTime > this._logInterval;
        if (shouldLog) {
            console.log(`%cStructuralShadowsLayer DEBUG | _onAnimate Periodic Log`, 'color: orange');
            console.log(`StructuralShadowsLayer DEBUG | Coords: camera_offset=(${topLeft.x.toFixed(2)}, ${topLeft.y.toFixed(2)}), view_size=(${viewSize[0].toFixed(2)}, ${viewSize[1].toFixed(2)})`);
            console.log(`StructuralShadowsLayer DEBUG | Filter Parallax Value: ${this.structuralFilter.uniforms.u_parallax.toFixed(4)}`);
        }
        
        this.intensityNoiseManager.update(deltaTime, renderer);
        
        if (this._needsOutdoorsMaskUpdate) {
            if (shouldLog) console.log("StructuralShadowsLayer DEBUG | Rendering outdoors mask.");
            renderer.render(this.outdoorsMaskContainer, { renderTexture: this.outdoorsMaskTexture, transform: stage.transform.worldTransform, clear: true });
            this._needsOutdoorsMaskUpdate = false;
        }

        if (this.parallaxMaskFilter) {
            if (shouldLog) console.log("StructuralShadowsLayer DEBUG | Rendering clean parallax mask.");
            ...
            renderer.render(this._parallaxMaskSprite, { renderTexture: this.cleanStructuralLightMask, clear: true });
        }
        ...
        // It enables the effect in the shader only if both the config setting is true AND the buffer is ready.
        u.u_illum_enabled = illumConfig.enabled && !!illumTexture?.valid;
        if (u.u_illum_enabled) {
            u.uIlluminationBuffer = illumTexture;
        }
        ...
        if (shouldLog) {
            ...
            console.log(`%cStructuralShadowsLayer DEBUG | _onAnimate Frame End`, 'color: orange');
        }
    }

    async updateEffectTargets(targets) {
        console.log("StructuralShadowsLayer DEBUG | updateEffectTargets: Called.");
        await super.updateEffectTargets(targets);
        if (!this.outdoorsMaskContainer) {
            console.warn("StructuralShadowsLayer DEBUG | updateEffectTargets: Aborted, outdoorsMaskContainer not ready.");
            return;
        }

        const structuralTargetCount = this.maskSprites.size;
        console.log(`StructuralShadowsLayer DEBUG | updateEffectTargets: Found ${structuralTargetCount} structural targets.`);
        ...
        const outdoorTargetCount = this.outdoorsMaskSprites.size;
        console.log(`StructuralShadowsLayer DEBUG | updateEffectTargets: Found ${outdoorTargetCount} outdoor targets.`);
        this._needsOutdoorsMaskUpdate = true;
    }

    async updateFromConfig(config) {
        const ssConfig = config.structuralShadows;
        console.log(`%cStructuralShadowsLayer DEBUG | updateFromConfig: Applying new config. Parallax: ${ssConfig.parallax}, Visible: ${config.enabled && ssConfig.enabled}`, 'color: cyan; font-weight: bold;');
    ...
    async _tearDown(options) {
        console.log(`%cStructuralShadowsLayer DEBUG | _tearDown: STARTING TEARDOWN for scene ${canvas.scene?.id}`, 'color: red; font-weight: bold;');
        
        const destroyAndLog = (obj, name) => {
            if (obj && typeof obj.destroy === 'function') {
                // The destroy signature for PIXI.Container is different, taking an options object.
                if (obj instanceof PIXI.Container) {
                    obj.destroy({ children: true, texture: true, baseTexture: true });
                } else {
                    obj.destroy();
                }
                console.log(`StructuralShadowsLayer DEBUG | _tearDown: Destroyed ${name}.`);
            }
        };
        ...
        console.log("StructuralShadowsLayer DEBUG | _tearDown: All PIXI objects destroyed. Nullifying properties.");
        this.intensityNoiseManager = null;
        this.structuralFilter = null;
        this._patternGeneratorSprite = null;
        this.finalShadowTexture = null;
        ...
        this._parallaxMaskSprite = null;
        await super._tearDown(options);
        console.log(`%cStructuralShadowsLayer DEBUG | _tearDown: FINISHED TEARDOWN`, 'color: red; font-weight: bold;');
    }
}
...
class DebuggerEventHandler {
    ...
    addEventListeners() {
        ...
        // Delegated listener for the hint manager
        this.element.addEventListener('click', this._handleListManagerClick.bind(this));
...
Hooks.once('init', () => {
    ...
    game.mapShine = {
        ...
        sceneChangeManager: new SceneChangeManager(), // Add the new manager instance
        debugger: null,
    ...
    if (game.modules.get('lib-wrapper')?.active) {
        libWrapper.register(MODULE_ID, 'Scene.prototype.view', async function(wrapped, ...args) {
            ...
            // A brief pause for the new canvas to initialize is still a good idea.
