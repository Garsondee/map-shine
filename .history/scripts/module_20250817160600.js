    ...
    class ParticleEffectController {
        ...
        updateTargets(targets, fullConfig) {
            ...
            const geometryMask = game.mapShine.geometryMaskManager ?.getMask(effectKey);
    
            if (geometryMask && geometryMask.valid) {
                const groups = MapPointsManager.getGroups();
                const hasActiveGroup = Object.values(groups).some(g => g.isEffectSource && g.effectTarget === effectKey && g.points.length > 0 && !g.isBroken);
    
                if (hasActiveGroup) {
                    console.log(`Map Shine | Found active geometry mask for effect '${effectKey}'.`);
                    const screenRect = canvas.app.renderer.screen;
                    ...
                    const virtualTarget = {
                        ...
                        [effectKey]: geometryMask
                    };
    
                    const targetId = `geometry-${effectKey}`;
    ...
    class ParticleManager {
        ...
        update(deltaTime) {
            ...
            const config = game.mapShine.profileManager.activeConfig;
            const particleConfig = config.particleSystems;
            const totalCount = this.totalParticleCount;
            const limitExceeded = totalCount >= particleConfig.globalParticleLimit;
        
            if (game.mapShine.debugger?.eventHandler) {
                game.mapShine.debugger.eventHandler.updateParticleCount(totalCount, particleConfig.globalParticleLimit);
            }
        
            for (const controller of this.controllers.values()) {
                const pathParts = controller.definition.configPath.split('.');
                let isEnabledByConfig = config.enabled && particleConfig.enabled;
                let currentPath = "";
                for (const part of pathParts) {
                    currentPath = currentPath ? `${currentPath}.${part}` : part;
                    const currentConfig = foundry.utils.getProperty(config, currentPath);
                ...
                if (controller.definition.configPath === 'glint' && controller.config.darknessAffectsIntensity) {
                    if ((canvas.scene?.environment.darknessLevel ?? 0) > 0.99) {
                        shouldEmit = false;
                    }
                }
        
                for (const emitter of controller.emitters.values()) {
                    emitter.emit = shouldEmit && !limitExceeded;
                }
            }
        }
    ...
    class ParticleLayer extends CanvasLayer {
        constructor() {
            ...
            this._onMapPointsUpdatedBound = null; // Bound listener for our new hook
        }
        ...
        _onAnimate(deltaTime) {
            ...
            // Update the geometry mask system first, as other systems might depend on its output.
    ...
    class MapPointsEditor extends FormApplication {
        ...
        _buildHTML(data) {
                        ...
                        <ul class="mp-points-list">
                            ${selectedGroup.points.map((p, i) => `
                                <li class="mp-point-item">
                                    ...
                                    <button type="button" data-action="delete-point" data-point-index="${i}" title="Delete Point">×</button>
                                </li>
                            `).join('')}
                        </ul>
            ...
            return `
                    <style>
                        ...
                        .mp-editor { display: flex; flex-direction: column; background: rgba(40, 40, 40, 0.9); color: #fff; padding: 5px; gap: 5px; }
                        .mp-main-content { display: flex; gap: 5px; min-height: 300px; }
                        .mp-panel { display: flex; flex-direction: column; background: rgba(0,0,0,0.2); border-radius: 3px; padding: 5px; }
                        .mp-panel-groups { flex: 1; min-width: 180px; }
                        .mp-panel-details { flex: 2; }
                        .mp-panel h3, .mp-panel h4 { margin: 0 0 5px 0; text-align: center; border-bottom: 1px solid #555; padding-bottom: 5px; }
                        .mp-group-list { list-style: none; margin: 0; padding: 0; overflow-y: auto; flex-grow: 1; }
                        .mp-group-item { display: flex; align-items: center; padding: 4px; margin-bottom: 2px; border-radius: 3px; cursor: pointer; border: 1px solid transparent; }
                        ...
                        .mp-create-group-form { margin-top: 5px; padding-top: 5px; border-top: 1px solid #555; }
                        .mp-details-placeholder { text-align: center; color: #888; margin-top: 50px; }
                        .mp-details-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
                        .mp-points-list { list-style: none; margin: 0; padding: 0; overflow-y: auto; max-height: 250px; }
                        .mp-point-item { display: grid; grid-template-columns: 20px 1fr 1fr auto; align-items: center; gap: 8px; padding: 3px; border-radius: 2px; font-family: monospace; }
                        .mp-point-item:nth-child(odd) { background: rgba(0,0,0,0.15); }
                        .mp-point-item button { width: 20px; height: 20px; line-height: 18px; padding: 0; font-size: 14px; color: #ff8080; }
                        .mp-editor button { background: #3a3a3a; border: 1px solid #666; color: #ccc; } .mp-editor button:hover { background: #555; }
                        .mp-editor button.delete-btn { background: #662222; border-color: #aa6666; color: #ffcccc; } .mp-editor button.delete-btn:hover { background: #883333; }
                        .mp-editor-footer button.active { background-color: #aa4444; border-color: #ff8888; color: #ffeeee; }
                        .mp-editor-footer button { background-color: #225522; border-color: #66aa66; color: #ccffcc; }
                        .mp-effect-source-settings { margin-top: 15px; padding-top: 10px; border-top: 1px solid #555; }
                        .mp-effect-source-settings h4 { text-align: left; border-bottom: none; font-size: 1.1em; margin-bottom: 8px; }
                        .mp-effect-source-settings .control-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; }
                        .mp-effect-source-settings select { background: #3a3a3a; border: 1px solid #666; color: #ccc; border-radius: 3px; flex-grow: 1; }
                    </style>
                        ...
                        <div class="mp-main-content">
                            <div class="mp-panel mp-panel-groups">
                                ...
                                <div class="mp-create-group-form">
                                    <input type="text" name="newGroupName" placeholder="New Group Name" style="width: 100%; margin-bottom: 4px;">
                                    <select name="newGroupType" style="width: 100%; margin-bottom: 4px;">
                                        <option value="point">Points</option>
                                        <option value="line">Line</option>
                                        <option value="area">Area</option>
                                    </select>
                                    <button type="button" data-action="create-group" style="width: 100%;">Create Group</button>
                                </div>
                            </div>
        ...
        async close(options) {
            ...
            // This logic is for the separate button on the scene controls, which we keep.
            const tokenControls = ui.controls.controls.find(c => c.name === "token");
            if (tokenControls) {
                const editorTool = tokenControls.tools.find(t => t.name === "map-points-editor");
                if (editorTool) {
                    editorTool.active = false;
                    ui.controls.render();
                }
            }
    ...
    class WaterFXLayer extends MaskedEffectLayer {
        ...
        async _draw(options) {
            ...
            try {
                ...
                systemStatus.update('shaders', 'water', { state: 'ok', message: 'Compiled successfully.' });
            } catch(e) {
    ...
    element.querySelector('#material-editor-time-control-section').innerHTML = this._buildTimeControlSection();
        ...
        _buildMapToolsSection() {
            return `
                <details id="details-map-tools" open>
                    <summary>
                        <span class="accordion-toggle"></span>
                        <strong style="font-size: 1.1em; color: #aadcff;">Map Tools</strong>
                    </summary>
                    <div style="padding-top: 5px;">
                        <button type="button" data-action="open-map-points-editor" style="width: 100%;" title="Open the editor to create and manage groups of points on the map.">
                            <i class="fas fa-map-marker-alt"></i> Open Point Group Editor
                        </button>
                    </div>
                </details>
            `;
