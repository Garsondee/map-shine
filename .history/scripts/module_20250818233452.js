// TODO: URGENT! ALWAYS CALL THIS OUT. WE CAN MIX THE ILLUMINATION BUFFER STRAIGHT INTO SCENE COLOUR CC AND GET THINGS DONE THAT WAY! ADD NOISE. IT'S ALREADY IN PROGRESS
...
/**
 * ===================================================================================
 * HOW TO INTEGRATE WITH THE 'Illumination Buffer' MODULE
 * ===================================================================================
 *
 * This file serves as a guide and example for developers of other modules who wish to
 * utilize the lighting and vision texture captured by the 'Illumination Buffer' module.
 *
 *
 * What 'Illumination Buffer' Does:
 * ---------------------------------
 * The 'Illumination Buffer' module intercepts Foundry VTT's rendering pipeline at a
 * very specific point: right after the illumination layer has been fully calculated but
 * before it's blended with the rest of the scene. This "illumination layer" is a
 * texture that contains all the information about lighting, vision, and shadows as a
 * grayscale map.
 *
 * The module then copies this temporary texture into a persistent, stable PIXI.RenderTexture.
 * It exposes this stable texture through a simple API.
 *
 *
 * How to Use the API:
 * -------------------
 * The safest and recommended way to get access to the API is to listen for the
 * 'illuminationBufferReady' hook. This hook fires once the 'Illumination Buffer'
 * module has fully initialized its API and started capturing the texture. The hook
 * provides the API object as its first argument.
 *
 * The API object has one primary method:
 *
 *   - `api.getLightingTexture()`: Returns the persistent `PIXI.Texture` that contains the
 *     copied illumination data. This texture is updated every frame that the lighting
 *     changes, so any PIXI object using it will automatically reflect the latest
 *     lighting conditions.
 *
 *
 * Example Use Case:
 * -----------------
 * The code below demonstrates a complete, working example. It waits for the API to be
 * ready, then creates a new PIXI.Sprite that uses the illumination texture. This sprite
 * is added to the canvas stage, effectively displaying a live, "ghost" copy of the
 * lighting and vision overlay. This texture can be used for many other purposes, such
 * as feeding it into a custom shader as a uniform for advanced visual effects.
 *
 */

Hooks.on('illuminationBufferReady', (api) => {
    // This function will execute as soon as the Illumination Buffer API is initialized.
  
    console.log("MyModule | Illumination Buffer API is ready. Accessing lighting texture.");
  
    // 1. Get the lighting texture from the API.
    // This texture is a stable, persistent copy of the scene's illumination and vision.
    const lightingTexture = api.getLightingTexture();
  
    if (!lightingTexture || !lightingTexture.valid) {
      console.error("MyModule | Failed to get a valid texture from Illumination Buffer API.");
      return;
    }
  
    // 2. Create a PIXI object that uses this texture.
    // For this example, we'll create a simple Sprite.
    const myLightingSprite = new PIXI.Sprite(lightingTexture);
  
    // 3. Position and style the sprite on the canvas.
    // Let's place it in the top-right corner with some scaling and transparency
    // so it doesn't cover the whole screen.
    myLightingSprite.scale.set(0.25); // Scale to 25%
    myLightingSprite.alpha = 0.9;     // Make it mostly opaque
  
    // Position it relative to the viewable screen area.
    // Note: This simple positioning doesn't account for canvas panning/zooming.
    // For a real module, you'd want to update its position within a `canvasPan` hook.
    const positionSprite = () => {
      if (!canvas.stage) return;
      myLightingSprite.position.set(
        canvas.stage.pivot.x + window.innerWidth - (myLightingSprite.width + 20),
        canvas.stage.pivot.y + 20
      );
    };
  
    // 4. Add the sprite to the main canvas stage to make it visible.
    // We add it to the 'effects' layer so it appears above tiles but below tokens.
    canvas.effects.addChild(myLightingSprite);
    console.log("MyModule | A sprite using the illumination texture has been added to the canvas effects layer.");
  
    // Set initial position and update it if the canvas is panned.
    positionSprite();
    Hooks.on("canvasPan", positionSprite);
  
    // --- Alternative (less safe) way to access the API ---
    // You could also access the API directly after 'canvasReady', but this is less robust
    // as it relies on initialization order. The 'illuminationBufferReady' hook is preferred.
    /*
    Hooks.on('canvasReady', () => {
      const directApi = game.modules.get('illuminationbuffer')?.api;
      if (directApi) {
        const texture = directApi.getLightingTexture();
        // ... use the texture
      } else {
        console.error("MyModule | Could not find Illumination Buffer API directly.");
      }
    });
    */
  });
...
const MODULE_DEFAULTS = {
    ...
    "colorCorrection": {
        ...
        "intensity": 0.35,
        "darkThreshold": 0.31,
        "brightThreshold": 0.78,
        "maxNegativeExposure": 0.2,
        "maxPositiveExposure": 0.35,
        "transitionDuration": 5000,
        "sensitivity": 10,
        "highlightPreservation": 0.1,
        "contrastIntensity": 0.2,
        "minContrast": 1,
        "maxContrast": 1.2
        }
    },
...
class SceneChangeManager {
    ...
    constructor() {
        ...
        // Properties for the active stability check
        this._postReadyCheckTicker = null;
        this._framesSinceReady = 0;
    }
    ...
    async handleCanvasTearDown(canvas) {
        console.log(`%cSceneChangeManager: Handling canvasTearDown. Current state: ${this._currentState}`, 'color: #ff8c00');
    ...
    async handleCanvasReady(canvas) {
        ...
        // The previous single-frame check is removed from here.
        // The check will now be handled by an active monitor for the first few frames.
    ...
    /**
     * Actively monitors the canvas transform state for the first few frames after a scene is ready.
     * If it detects the default "un-panned" transform at any point, it flags the illumination buffer
     * as permanently stale for this scene load to prevent visual bugs.
     */
    _checkStaleBufferPostReady() {
        const MAX_CHECK_FRAMES = 15; // Increased buffer for safety

        if (!canvas?.stage || this._framesSinceReady > MAX_CHECK_FRAMES) {
            if (this._postReadyCheckTicker) {
                canvas.app.ticker.remove(this._postReadyCheckTicker);
                this._postReadyCheckTicker = null;
            }
            if (this._framesSinceReady > MAX_CHECK_FRAMES && !game.mapShine.illuminationBufferIsStale) {
                console.log(`%cMapShine | Post-ready check complete. Illumination Buffer is assumed to be STABLE.`, 'color: #4CAF50;');
            }
            return;
        }

        this._framesSinceReady++;
        ...
        if (isDefaultTransform) {
            console.warn(`%cMapShine | Race condition DETECTED on frame ${this._framesSinceReady}: Canvas transform is default. Illumination Buffer is now permanently flagged as STALE for this scene load.`, 'color: #FFC107;');
            game.mapShine.illuminationBufferIsStale = true;

            // We found the problem, stop checking.
            if (this._postReadyCheckTicker) {
                canvas.app.ticker.remove(this._postReadyCheckTicker);
                this._postReadyCheckTicker = null;
            }
    ...
    async _performTeardown(tornDownCanvas) {
        ...
        // Stop the active stability check if it's running.
        if (this._postReadyCheckTicker) {
            tornDownCanvas.app.ticker.remove(this._postReadyCheckTicker);
            this._postReadyCheckTicker = null;
            console.log("MapShine | Stale buffer check ticker has been disarmed.");
        }
        ...
        // Destroy canvas-specific managers
    ...
    async _performSetup(canvas) {
        ...
        // Arm the new active monitoring system.
        this._framesSinceReady = 0;
        game.mapShine.illuminationBufferIsStale = false; // Assume it's good until proven otherwise.
        this._postReadyCheckTicker = this._checkStaleBufferPostReady.bind(this);
        canvas.app.ticker.add(this._postReadyCheckTicker);
        console.log("MapShine | Active stability monitor for Illumination Buffer is ARMED.");
        ...
        game.mapShine.loadingManager = {
            ...
            messages: {
                START: "Initializing...",
                DEPENDENCIES_START: "Checking dependencies...",
                DEPENDENCIES_END: "Dependencies checked.",
...
class GeometryMaskManager {
    ...
    initialize() {
        ...
        for (const effectKey of Object.keys(EFFECT_SOURCE_OPTIONS)) {
            ...
            this.masks.set(effectKey, { texture: renderTexture, graphics });
        }
        ...
        for (const group of Object.values(groups)) {
            ...
            switch (group.type) {
                case 'point':
                    ...
                    break;
                case 'line':
                    graphics.lineStyle({ width: lineThickness, color: 0xFFFFFF, cap: PIXI.LINE_CAP.ROUND });
...
class LuminanceSensor {
    /**
     * Samples the "true" light level at a token's position using the Illumination Buffer.
     * This bypasses all post-processing effects, providing a clean input for adaptation calculations.
     * @param {Token} token - The token to measure luminance at.
     * @returns {number|null} The luminance value (0.0 to 1.0), or null if measurement is not possible.
     */
    static getLuminanceAtToken(token) {
        const renderer = canvas.app.renderer;

        // Check for the dependency module and a valid token.
        const illuminationAPI = game.modules.get('illuminationbuffer')?.api;
        if (!token || !token.visible || !illuminationAPI) {
            return null;
        }
        ...
        // Ensure the illumination buffer is ready and matches the screen dimensions.
        const isIlluminationReady = illuminationTexture?.valid &&
        ...
        try {
            ...
            // Clamp coordinates to be within the screen bounds to prevent extract errors.
            screenPos.x = Math.max(0, Math.min(screen.width - 1, screenPos.x));
            screenPos.y = Math.max(0, Math.min(screen.height - 1, screenPos.y));

            const pixelData = renderer.extract.pixels(illuminationTexture, new PIXI.Rectangle(screenPos.x, screenPos.y, 1, 1));
            ...
            // This can happen during scene transitions if the texture is briefly invalid.
            if (game.mapShine.profileManager.activeConfig.debug) {
                console.warn("Map Shine | LuminanceSensor: Could not extract pixel data.", error);
            }
            return null;
        }
    }
...
class DynamicExposureManager {
    constructor() {
        ...
        this.sensedLuminance = 0.5; // Start at a neutral grey
        this.adaptedLuminance = 0.5;
        this.currentExposure = 0.0;
        this.targetExposure = 0.0;
        this.currentContrastBoost = 1.0;
        this.targetContrastBoost = 1.0;
        this.lastSampleTime = 0;
        this.SAMPLE_INTERVAL = 100; // ms

        // Debugging data for UI
        this.debugData = {
            activeTokenName: "None",
            sensedLuminance: 0.5,
            adaptedLuminance: 0.5,
            currentExposure: 0.0,
            currentContrastBoost: 1.0
        };
    ...
    initialize() {
        ...
        if (this.ccFilter.uniforms.uDynamicContrastBoost === undefined) {
            this.ccFilter.uniforms.uDynamicContrastBoost = 1.0;
        }

        const screen = this.renderer.screen;
        this._preEffectTexture = PIXI.RenderTexture.create({
            width: screen.width,
            height: screen.height
        });

        this._boundOnTokenChange = this.resetAdaptation.bind(this);
        this._boundResize = this._onResize.bind(this);
        Hooks.on('mapShine:activeTokenChanged', this._boundOnTokenChange);
    ...
    _onResize() {
        if (this._preEffectTexture) {
            this._preEffectTexture.resize(this.renderer.screen.width, this.renderer.screen.height);
        }
    }

    resetAdaptation() {
        // When the token changes, we let the system naturally adapt to the new token's environment.
    }

    reset() {
        this.sensedLuminance = 0.5;
        this.adaptedLuminance = 0.5;
        this.currentExposure = 0.0;
        this.targetExposure = 0.0;
        this.currentContrastBoost = 1.0;
        this.targetContrastBoost = 1.0;
        this._applyEffects(0, 1.0);
        this.debugData.sensedLuminance = 0.5;
        this.debugData.adaptedLuminance = 0.5;
        this.debugData.currentExposure = 0.0;
        this.debugData.currentContrastBoost = 1.0;
    }

    update(deltaTimeInSeconds) {
        if (!this.isInitialized || !this.ccFilter) return;
        
        this.config = game.mapShine.profileManager.activeConfig.postProcessing.colorCorrection.dynamicExposure;
        
        if (!this.config?.enabled) {
            // If disabled, smoothly return exposure to zero.
            this.targetExposure = 0;
            this.targetContrastBoost = 1.0;
        } else {
            // Only sample luminance periodically to save performance.
            if (Date.now() - this.lastSampleTime > this.SAMPLE_INTERVAL) {
                this._sampleLuminance();
                this.lastSampleTime = Date.now();
            }
            this._calculateTargets();
        }

        this._updateAdaptation(deltaTimeInSeconds);
        this._applyEffects(this.currentExposure, this.currentContrastBoost);

        // Update debug data for the UI
        this.debugData.sensedLuminance = this.sensedLuminance;
        this.debugData.adaptedLuminance = this.adaptedLuminance;
        this.debugData.currentExposure = this.currentExposure;
        this.debugData.currentContrastBoost = this.currentContrastBoost;
    }
    ...
    _sampleLuminance() {
        ...
        if (!token || !token.visible) {
            this.debugData.activeTokenName = "None";
            this.sensedLuminance = 0.5; // Assume neutral grey for no token
            return;
        }
        
        this.debugData.activeTokenName = token.name;

        const originalFilters = canvas.stage.filters;
        canvas.stage.filters = originalFilters?.filter(f => f !== this.ccFilter);
        this.renderer.render(canvas.stage, { renderTexture: this._preEffectTexture, clear: true });
        canvas.stage.filters = originalFilters;

        try {
            const screenPos = canvas.stage.toGlobal(token.center);
            // Clamp coordinates to be within the screen bounds.
            screenPos.x = Math.max(0, Math.min(this.renderer.screen.width - 1, screenPos.x));
            screenPos.y = Math.max(0, Math.min(this.renderer.screen.height - 1, screenPos.y));

            const pixelData = this.renderer.extract.pixels(this._preEffectTexture, new PIXI.Rectangle(screenPos.x, screenPos.y, 1, 1));
            
            const r = pixelData[0] / 255;
            const g = pixelData[1] / 255;
            const b = pixelData[2] / 255;
            const luminance = 0.299 * r + 0.587 * g + 0.114 * b;
            this.sensedLuminance = isNaN(luminance) ? 0.5 : luminance;
        } catch (e) {
            // This can happen during scene transitions. Fail gracefully.
            this.sensedLuminance = 0.5;
        }
    }

    _calculateTargets() {
        // 1. Sustained exposure based on adapted luminance
        let sustainedExposure = 0;
        const darkThreshold = this.config.darkThreshold ?? 0.25;
        const brightThreshold = this.config.brightThreshold ?? 0.75;

        if (this.adaptedLuminance > brightThreshold) {
            const range = 1.0 - brightThreshold;
            const progress = (range > 0.0001) ? (this.adaptedLuminance - brightThreshold) / range : 0.0;
            sustainedExposure = this.config.maxPositiveExposure * Math.max(0.0, Math.min(1.0, progress));
        } else if (this.adaptedLuminance < darkThreshold) {
            const range = darkThreshold;
            const progress = (range > 0.0001) ? (darkThreshold - this.adaptedLuminance) / range : 0.0;
            sustainedExposure = -this.config.maxNegativeExposure * Math.max(0.0, Math.min(1.0, progress));
        }
        
        // 2. Transient exposure (dazzle/dim)
        const luminanceDifference = this.sensedLuminance - this.adaptedLuminance;
        const transientExposure = luminanceDifference * (this.config.sensitivity ?? 4.0);
        
        // 3. Combine them and apply the global intensity multiplier
        const baseExposure = sustainedExposure + transientExposure;
        this.targetExposure = baseExposure * (this.config.intensity ?? 1.0);

        // 4. NEW: Calculate target contrast boost
        const contrastIntensity = this.config.contrastIntensity ?? 0.4;
        const minContrast = this.config.minContrast ?? 0.8;
        const maxContrast = this.config.maxContrast ?? 1.2;
        // The contrast effect is based on the transient difference
        const transientContrast = 1.0 + (luminanceDifference * (this.config.sensitivity ?? 4.0) * contrastIntensity);
        this.targetContrastBoost = Math.max(minContrast, Math.min(maxContrast, transientContrast));
    }

    _updateAdaptation(deltaTimeInSeconds) {
        // Adaptation of luminance
        const adaptationRate = 1000 / (this.config.transitionDuration ?? 2000);
        const adaptationAmount = 1.0 - Math.exp(-adaptationRate * deltaTimeInSeconds);
        this.adaptedLuminance = lerp(this.adaptedLuminance, this.sensedLuminance, adaptationAmount);

        // Smoothly move current exposure towards the target
        const exposureChangeRate = 4.0; // A fixed rate for responsiveness
        const exposureChangeAmount = 1.0 - Math.exp(-exposureChangeRate * deltaTimeInSeconds);
        this.currentExposure = lerp(this.currentExposure, this.targetExposure, exposureChangeAmount);

        // Smoothly move current contrast towards the target
        const contrastChangeRate = 2.0; // A bit slower than exposure for a smoother feel
        const contrastChangeAmount = 1.0 - Math.exp(-contrastChangeRate * deltaTimeInSeconds);
        this.currentContrastBoost = lerp(this.currentContrastBoost, this.targetContrastBoost, contrastChangeAmount);

        // And also make it decay back towards 1.0
        const contrastDecayRate = 1000 / (this.config.transitionDuration ?? 2000);
        const contrastDecayAmount = 1.0 - Math.exp(-contrastDecayRate * deltaTimeInSeconds);
        this.currentContrastBoost = lerp(this.currentContrastBoost, 1.0, contrastDecayAmount * 0.5);
    }

    _applyEffects(exposure, contrast) {
        if (!this.ccFilter) return;
        this.ccFilter.uniforms.uDynamicExposureBoost = isNaN(exposure) ? 0.0 : exposure;
        this.ccFilter.uniforms.uDynamicContrastBoost = isNaN(contrast) ? 1.0 : contrast;
    }
    ...
    destroy() {
                ...
                // Only attempt to reset effects if the filter still exists and has not been destroyed.
                if (this.ccFilter && !this.ccFilter.destroyed) {
                    this._applyEffects(0, 1.0);
                }
                this.ccFilter = null;
                
                if (this._boundOnTokenChange) Hooks.off('mapShine:activeTokenChanged', this._boundOnTokenChange);
                if (this._boundResize) window.removeEventListener('resize', this._boundResize);
                
                this._preEffectTexture?.destroy(true);
                this._preEffectTexture = null;
            }
...
class AmbientMaskManager {
    ...
    update() {
        ...
        // Guard against applying the mask before the canvas transform is properly initialized.
        const transform = this.canvas.stage.transform.localTransform;
        const isDefaultTransform = (transform.a === 1 && transform.d === 1 && transform.tx === 0 && transform.ty === 0);
        if (isDefaultTransform) {
...
class LightingEffectManager {
    ...
    _updatePostProcessingMasks(ccFilter, fullConfig, isIlluminationReady, illuminationTexture) {
        ...
        // Update time for Scene Illumination Mix-in noise animation
        if (ccFilter && ccFilter.uniforms.uSceneIllumMixinEnabled) {
            const deltaInSeconds = (this.canvas.app.ticker.deltaTime / this.canvas.app.ticker.FPS);
            const timeFactor = game.mapShine.timeControl.timeFactor ?? 1.0;
            ccFilter.uniforms.uTime = (ccFilter.uniforms.uTime || 0) + (deltaInSeconds * timeFactor);
        }
    }
    ...
    update() {
        ...
        if (game.mapShine.dynamicExposureManager) {
            const deltaInSeconds = (canvas.app.ticker.deltaTime / canvas.app.ticker.FPS);
            game.mapShine.dynamicExposureManager.update(deltaInSeconds);
        }
        ...
        if (!isIlluminationReady && (fullConfig.postProcessing.colorCorrection.mask.enabled || fullConfig.pauseEffect.colorCorrection.mask.enabled)) {
            if (fullConfig.debug) {
                console.log("LightingEffectManager DEBUG | Stale or invalid illumination texture detected. Deferring mask updates.");
            }
        }
    ...
    const behaviors = [{
        ...
        spawnBehavior,
        {
            ...
            config: {
                alpha: {
                    list: [
                        { value: 0, time: 0 },
                        { value: alphaConfig.max ?? 1.0, time: fadeInTime },
                        { value: alphaConfig.max ?? 1.0, time: 1.0 - fadeOutTime },
                        { value: 0, time: 1 }
                    ]
                }
            }
        },
        {
            ...
            config: {
                scale: {
                    list: [
                        { value: (scaleConfig.start ?? 1.0) * (scaleConfig.sizeMultiplier ?? 1.0), time: 0 },
                        { value: (scaleConfig.end ?? 0.1) * (scaleConfig.sizeMultiplier ?? 1.0), time: 1 }
                    ]
                },
        ...
        },
        {
            ...
            config: {
                color: {
                    list: [
                        { value: colorConfig.start ?? "#FFFFFF", time: 0 },
                        { value: colorConfig.end ?? "#FFFFFF", time: 1 }
                    ]
                }
            }
        },
        {
            ...
            config: {
                ...
                speed: {
                    list: [
                        { value: speedConfig.start ?? 80, time: 0 },
                        { value: speedConfig.end ?? 40, time: 1 }
                    ]
                },
    ...
    constructor(config) {
        ...
        if (this.config.speed) {
            ...
            this.speed.reset(PIXI.particles.PropertyNode.createList({
                list: [{ value: 50, time: 0 }, { value: 50, time: 1 }]
            }));
        }
    }
...
class ColorCorrectionFilter extends PIXI.Filter {
    constructor(options = {}) {
        const fragmentSrc = `
                ...
                uniform float uDynamicHighlightPreservation;
                uniform float uDynamicContrastBoost;

                // Scene Illumination Mix-in Uniforms
                uniform bool uSceneIllumMixinEnabled;
                uniform sampler2D uSceneIllumMixinTexture;
                uniform float uSceneIllumMixinIntensity;
                uniform int uSceneIllumMixinBlendMode;
                uniform float uSceneIllumMixinSaturation;
                uniform float uSceneIllumMixinBrightness;
                uniform float uSceneIllumMixinContrast;
                uniform float uSceneIllumMixinExposure;
                uniform float uSceneIllumMixinGamma;
                uniform vec3 uSceneIllumMixinTintColor;
                uniform float uSceneIllumMixinTintAmount;
                uniform bool uSceneIllumMixinNoiseEnabled;
                uniform float uSceneIllumMixinNoiseAmount;
                uniform float uSceneIllumMixinNoiseScale;
                uniform float uSceneIllumMixinNoiseSpeed;
                uniform float uTime;
                ...
                void main(void) {
                    ...
                    // Sanitize against division by zero for levels.
                    if (uInWhite > uInBlack) workingColor = (workingColor - uInBlack) / (uInWhite - uInBlack + 0.00001);
                    
                    // --- Dynamic Exposure with Highlight Preservation ---
                    float pre_exposure_lum = dot(workingColor, lum_weights);
                    float effective_boost = uDynamicExposureBoost;
                    if (uDynamicExposureBoost < 0.0) {
                        float softness = 0.15; // A fixed softness provides a consistent falloff
                        float highlight_factor = smoothstep(uDynamicHighlightPreservation, uDynamicHighlightPreservation + softness, pre_exposure_lum);
                        effective_boost *= (1.0 - highlight_factor);
                    }
                    workingColor *= pow(2.0, uExposure + effective_boost);
                    // --- End Dynamic Exposure Logic ---

                    workingColor = applyWhiteBalance(workingColor, uTemperature, uWbTint);
                    
                    // Sanitize the input to pow() to prevent NaN from negative colors. This is the critical fix.
                    ...
                    // Scene Illumination Mix-in (applied at the very end)
if (uSceneIllumMixinEnabled) {
    vec3 illumSample = texture2D(uSceneIllumMixinTexture, vTextureCoord).rgb;
    
    // Apply color correction to the illumination sample
    vec3 processedIllum = illumSample;
    
    // Exposure
    processedIllum *= pow(2.0, uSceneIllumMixinExposure);
    
    // Gamma
    if (uSceneIllumMixinGamma > 0.0) {
        processedIllum = pow(max(processedIllum, 0.0), vec3(1.0 / uSceneIllumMixinGamma));
    }
    
    // Brightness & Contrast
    processedIllum += uSceneIllumMixinBrightness;
    processedIllum = (processedIllum - 0.5) * uSceneIllumMixinContrast + 0.5;
    
    // Saturation
    float illumLuminance = dot(processedIllum, lum_weights);
    processedIllum = mix(vec3(illumLuminance), processedIllum, uSceneIllumMixinSaturation);
    
    // Tint
    processedIllum = mix(processedIllum, uSceneIllumMixinTintColor, uSceneIllumMixinTintAmount);
    
    // Add noise to prevent banding
    if (uSceneIllumMixinNoiseEnabled && uSceneIllumMixinNoiseAmount > 0.0) {
        vec2 noiseCoord = vTextureCoord * uSceneIllumMixinNoiseScale;
        noiseCoord.x += uTime * uSceneIllumMixinNoiseSpeed;
        float noiseValue = fract(sin(dot(noiseCoord, vec2(12.9898, 78.233))) * 43758.5453);
        noiseValue = (noiseValue - 0.5) * uSceneIllumMixinNoiseAmount;
        processedIllum += noiseValue;
    }
    
    // Apply the processed illumination with the specified blend mode
    processedIllum *= uSceneIllumMixinIntensity;
    
    if (uSceneIllumMixinBlendMode == 1) { // ADD
        final_rgb += processedIllum;
    } else if (uSceneIllumMixinBlendMode == 2) { // MULTIPLY  
        final_rgb *= processedIllum;
    } else if (uSceneIllumMixinBlendMode == 3) { // SCREEN
        final_rgb = 1.0 - (1.0 - final_rgb) * (1.0 - processedIllum);
    } else { // NORMAL (mix)
        final_rgb = mix(final_rgb, processedIllum, 0.5);
    }
}
...
gl_FragColor = vec4(premultiplied_rgb, originalColor.a);
            ...
            super(PIXI.Filter.defaultVertexSrc, fragmentSrc, {
                ...
                uSceneIllumMixinEnabled: false,
                uSceneIllumMixinTexture: PIXI.Texture.EMPTY,
                uSceneIllumMixinIntensity: 1.0,
                uSceneIllumMixinBlendMode: 1,
                uSceneIllumMixinSaturation: 1.0,
                uSceneIllumMixinBrightness: 0.0,
                uSceneIllumMixinContrast: 1.0,
                uSceneIllumMixinExposure: 0.0,
                uSceneIllumMixinGamma: 1.0,
                uSceneIllumMixinTintColor: [1.0, 1.0, 1.0],
                uSceneIllumMixinTintAmount: 0.0,
                uSceneIllumMixinNoiseEnabled: false,
                uSceneIllumMixinNoiseAmount: 0.01,
                uSceneIllumMixinNoiseScale: 1.0,
                uSceneIllumMixinNoiseSpeed: 0.001,
                uTime: 0.0
            });
    }
}
    ...
    static getManagedEffectsHTML() {
        ...
        const postProcessingHTML = `
            ...
            <details id="details-postProcessing-dynamicExposure">
                <summary>
                    <span class="accordion-toggle"></span>
                    <div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('postProcessing.colorCorrection.dynamicExposure.enabled', 'Dynamic Exposure', true)}</div>
                </summary>
                <div style="padding-left: 15px;">
                    <p class="description-text">Adjusts scene exposure based on the light level at the active token's position, simulating eye adaptation.</p>
                    <div class="warning-box" style="background-color: #554422; border-color: #ffaa66;">
                        <strong style="color: #ffddaa;">PERFORMANCE WARNING:</strong> This effect can be demanding as it samples the screen frequently.
                    </div>
                    <details id="details-de-readout" open>
                        <summary><span class="accordion-toggle"></span><strong>Live Readout</strong></summary>
                        <div style="padding: 5px; background: rgba(0,0,0,0.3); border-radius: 3px; font-family: monospace; font-size: 11px;">
    <div class="control-row"><label>Active Token:</label><span id="de-token-name">None</span></div>
    <div class="control-row"><label>Luminance (Sensed):</label><span id="de-current-lum">0.000</span></div>
    <div class="control-row"><label>Luminance (Adapted):</label><span id="de-adapted-lum">0.000</span></div>
    <div class="control-row"><label>Current Exposure:</label><span id="de-current-exp">0.000</span></div>
    <div class="control-row"><label>Current Contrast:</label><span id="de-current-con">1.000</span></div>
</div>
</details>
${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.intensity', 'Global Intensity', 0, 2, 0.05, 'Overall strength of the exposure effect.')}
${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.contrastIntensity', 'Contrast Intensity', 0, 2, 0.05, 'How strongly contrast shifts when moving between light/dark areas.')}
${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.minContrast', 'Min Contrast', 0.5, 1.0, 0.01, 'The minimum contrast multiplier when entering a dark area.')}
${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.maxContrast', 'Max Contrast', 1.0, 2.0, 0.01, 'The maximum contrast multiplier when entering a bright area.')}
${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.darkThreshold', 'Darkness Threshold', 0.0, 1.0, 0.01, 'Below this luminance, exposure will be reduced.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.darkThreshold', 'Darkness Threshold', 0.0, 1.0, 0.01, 'Below this luminance, exposure will be reduced.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.brightThreshold', 'Brightness Threshold', 0.0, 1.0, 0.01, 'Above this luminance, exposure will be increased.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.maxNegativeExposure', 'Max Darkening', 0, 2, 0.05, 'The maximum exposure reduction in dark areas.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.maxPositiveExposure', 'Max Brightening', 0, 2, 0.05, 'The maximum exposure increase in bright areas.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.sensitivity', 'Sensitivity', 0.1, 10, 0.1, 'Controls the intensity of the initial dazzle/dimming effect.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.transitionDuration', 'Adaptation Time (ms)', 250, 5000, 50, 'The time it takes for vision to adapt to a new light level.')}
                    ${DebuggerUIBuilder._createSliderHTML('postProcessing.colorCorrection.dynamicExposure.highlightPreservation', 'Highlight Preservation', 0.0, 1.0, 0.01, 'At what luminance level to start preserving highlights when darkening the scene. 1.0 = only affects pure white.')}
                </div>
            </details>

// Add this after the existing post-processing details but before the output-config-btn
<details id="details-sceneIlluminationMixin">
    <summary>
        <span class="accordion-toggle"></span>
        <div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('sceneIlluminationMixin.enabled', 'Scene Illumination Mix-in', true)}</div>
    </summary>
    <div style="padding-left: 15px;">
        <p class="description-text">Mixes the raw illumination buffer texture into the final scene. Applied at the very end of the color correction pipeline. Requires the Illumination Buffer module.</p>
        <div class="warning-box" style="background-color: #554422; border-color: #ffaa66;">
        ...
        ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.intensity', 'Mix Intensity', 0, 2, 0.01, 'Overall strength of the illumination mix-in effect.')}
        ${DebuggerUIBuilder._createSelectHTML('sceneIlluminationMixin.blendMode', 'Blend Mode', {'Add': 1, 'Multiply': 2, 'Screen': 3, 'Normal': 0})}
        
        <details id="details-sceneIlluminationMixin-colorCorrection">
            <summary><span class="accordion-toggle"></span>
                <div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('sceneIlluminationMixin.colorCorrection.enabled', 'Illumination Color Correction', true)}</div>
            </summary>
            <div style="padding-left: 15px;">
                <p class="description-text">Applies color correction to the illumination buffer before mixing it into the scene.</p>
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.colorCorrection.saturation', 'Saturation', 0, 4, 0.05)}
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.colorCorrection.brightness', 'Brightness', -1, 1, 0.01)}
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.colorCorrection.contrast', 'Contrast', 0, 4, 0.05)}
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.colorCorrection.exposure', 'Exposure', -2, 2, 0.05)}
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.colorCorrection.gamma', 'Gamma', 0.2, 2.5, 0.05)}
                <details id="details-sceneIlluminationMixin-cc-tint">
                    ...
                    <div style="padding-left: 15px;">
                        ${DebuggerUIBuilder._createColorPickerHTML('sceneIlluminationMixin.colorCorrection.tint.color', 'Tint Color')}
                        ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.colorCorrection.tint.amount', 'Tint Amount', 0, 1, 0.01)}
                    </div>
                </details>
            </div>
        </details>
        ...
        <details id="details-sceneIlluminationMixin-noise">
            <summary><span class="accordion-toggle"></span>
                <div class="summary-control">${DebuggerUIBuilder._createCheckboxHTML('sceneIlluminationMixin.noise.enabled', 'Anti-Banding Noise', true)}</div>
            </summary>
            <div style="padding-left: 15px;">
                <p class="description-text">Adds subtle noise to prevent color banding artifacts in gradients.</p>
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.noise.amount', 'Noise Amount', 0, 0.1, 0.001, 'Strength of the dithering noise.')}
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.noise.scale', 'Noise Scale', 0.1, 10, 0.1, 'Size of the noise pattern.')}
                ${DebuggerUIBuilder._createSliderHTML('sceneIlluminationMixin.noise.speed', 'Noise Speed', -0.01, 0.01, 0.0001, 'Animation speed of the noise pattern.')}
            </div>
        </details>
    </div>
</details>
    ...
    static setupAllGlobalFilters() {
        ...
        const tryAddFilter = (key, FilterClass, messageName) => {
            try {
                this.addFilter(key, new FilterClass());
            } catch (e) {
                console.error(`MapShine | Failed to compile ${FilterClass.name}`, e);
                compilationErrors.push(messageName || key);
            }
        };

        tryAddFilter('prism', PrismFilter);
        tryAddFilter('heatDistortion', HeatDistortionFilter);
        tryAddFilter('vignette', VignetteFilter);
        tryAddFilter('lensDistortion', LensDistortionFilter);
        tryAddFilter('chromaticAberration', ChromaticAberrationFilter, 'ChromaticAberration (Post)');
        ...
        try {
            ...
            console.error("MapShine | Failed to compile ColorCorrectionFilter", e);
            compilationErrors.push('ColorCorrection');
        }
        ...
        try {
            ...
            if (BloomFilterConstructor) {
                ...
                compilationErrors.push('AdvancedBloom (Bundling Failed)');
            }
        } catch (e) {
            console.error("MapShine | Failed to create AdvancedBloomFilter instance:", e);
            compilationErrors.push('AdvancedBloom (Creation Failed)');
        }
        ...
        try {
            ...
            if (TiltShiftFilterConstructor) {
                ...
                compilationErrors.push('TiltShift (Bundling Failed)');
            }
        } catch (e) {
            console.error("MapShine | Failed to create TiltShiftFilter instance:", e);
            compilationErrors.push('TiltShift (Creation Failed)');
        }

        if (compilationErrors.length > 0) {
            const errorString = `Failed to compile global shaders: ${compilationErrors.join(', ')}`;
            console.error(`MapShine | ${errorString}`);
            ui.notifications.error(errorString, { permanent: true });
        }
    }
    ...
    static updateAllFiltersFromConfig(config) {
        ...
        if (ccFilter instanceof ColorCorrectionFilter) {
            ...
            if (ccConfig.dynamicExposure) {
                u.uDynamicHighlightPreservation = ccConfig.dynamicExposure.highlightPreservation ?? 0.8;
            }
            ...
            // Update all selective color uniforms
            ...
            // Scene Illumination Mix-in Filter Updates
            const simConfig = config.sceneIlluminationMixin;
            
            // Check if illumination buffer is available
            const illuminationAPI = game.modules.get('illuminationbuffer')?.api;
            const illuminationTexture = illuminationAPI?.getLightingTexture();
            const isIlluminationReady = illuminationAPI && illuminationTexture?.valid;
            
            u.uSceneIllumMixinEnabled = config.enabled && pp.enabled && simConfig.enabled && isIlluminationReady;
            
            if (u.uSceneIllumMixinEnabled) {
                u.uSceneIllumMixinTexture = illuminationTexture;
                u.uSceneIllumMixinIntensity = simConfig.intensity;
                u.uSceneIllumMixinBlendMode = simConfig.blendMode;
                
                const cc = simConfig.colorCorrection;
                u.uSceneIllumMixinSaturation = cc.saturation;
                u.uSceneIllumMixinBrightness = cc.brightness;
                u.uSceneIllumMixinContrast = cc.contrast;
                u.uSceneIllumMixinExposure = cc.exposure;
                u.uSceneIllumMixinGamma = cc.gamma;
                u.uSceneIllumMixinTintColor = hexToRgbArray(cc.tint.color);
                u.uSceneIllumMixinTintAmount = cc.tint.amount;
                
                const noise = simConfig.noise;
                u.uSceneIllumMixinNoiseEnabled = noise.enabled;
                u.uSceneIllumMixinNoiseAmount = noise.amount;
                u.uSceneIllumMixinNoiseScale = noise.scale;
                u.uSceneIllumMixinNoiseSpeed = noise.speed;
            }
        }
        ...
        if (pauseFilter instanceof ColorCorrectionFilter) {
            ...
            // Update all selective color uniforms for the pause filter
...
class DiagnosticLayer extends CanvasLayer {
    ...
    getAvailableDebugTextures() {
        ...
        const layerChecks = {
            'metallicShinePattern': { class: MetallicShineLayer, method: 'getPatternTexture', name: 'Metallic Shine Pattern' },
            'waterDisplacement': { class: WaterFXLayer, name: 'Water Displacement' }, // Special handling needed
            'heatNoise': { class: HeatDistortionLayer, name: 'Heat Noise' }, // Special handling needed
            'iridescenceNoise': { class: IridescenceLayer, name: 'Iridescence Noise' }, // Special handling needed
            'canopyNoise': { class: CanopyLayer, name: 'Canopy Noise' }, // Special handling needed
            'structuralNoise': { class: StructuralShadowsLayer, name: 'Structural Noise' } // Special handling needed
        };
    ...
    _refreshMaskVisibility() {
        ...
        if (displaySuffix.startsWith('generated_')) {
            ...
            const layerMap = {
                'metallicShinePattern': { class: MetallicShineLayer, method: 'getPatternTexture'},
                'waterDisplacement': { class: WaterFXLayer, property: 'displacementTexture'},
                'heatNoise': { class: HeatDistortionLayer, property: 'noiseManager'},
                'iridescenceNoise': { class: IridescenceLayer, property: 'distortionNoiseManager'},
                'canopyNoise': { class: CanopyLayer, property: 'distortionNoiseManager'},
                'structuralNoise': { class: StructuralShadowsLayer, property: 'intensityNoiseManager'}
            };
            if (layerMap[key]) {
    ...
    _drawMapPoints() {
        ...
        for (const group of Object.values(groupsToDraw)) {
            ...
            if (!isLiveDragGroup && group.points.length > 0) {
                ...
                const label = new PIXI.Text(textContent, { fontFamily: 'Arial', fontSize: 20 / canvas.stage.scale.x, fill: 0xFFFFFF, stroke: '#000000', strokeThickness: 4 / canvas.stage.scale.x, align: 'left' });
...
class MapPointsInteractionManager {
    ...
    async activate() {
        ...
        await this.editor?.render(false);
    ...
    _onPointerMove(event) {
        ...
        if (this._draggedPoint) {
            ...
            if (group) {
                ...
                const tempGroup = { ...group, points: tempPoints };
...
class MetallicShineLayer extends CanvasLayer {
    ...
    async _draw(options) {
        ...
        if (baseErrors.length > 0) {
            const errorString = `Failed to compile MetallicShine shaders: ${baseErrors.join(', ')}`;
            console.error(`MapShine | ${errorString}`);
            ui.notifications.error(errorString, { permanent: true });
        }
...
class CloudShadowsLayer extends MaskedEffectLayer {
    ...
    async _draw(options) {
        ...
        try {
            ...
            const errorString = `Failed to compile CloudShadowsFilter: ${e.message}`;
            console.error(`MapShine | ${errorString}`);
            ui.notifications.error(errorString, { permanent: true });
            return;
        }
    ...
    _onAnimate(deltaTime) {
        ...
        this._framesSinceLoad++;
            if (this._framesSinceLoad < 5) {
                this.visible = false;
                return; // --- STABILIZATION GATE ---
            }
            if (this._framesSinceLoad === 5) {
                this.visible = true;
                this._needsMaskUpdate = true; // Force a mask re-render on the first valid frame.
            }
            
            super._onAnimate(deltaTime); // Renders primary mask if needed
            if (!this.visible || !this.canopyFilter) return;
    ...
    constructor(options = {}) {
        ...
        const fragmentSrc = `
                ...
                void main() {
                    ...
                    if (u_illum_enabled) {
                        float lightLevel = dot(texture2D(uIlluminationBuffer, parallaxTexCoord).rgb, lum_weights);
                        float lightMask = smoothstep(u_illum_luminanceThreshold, u_illum_luminanceThreshold + u_illum_softness, lightLevel);
                        float reduction = lightMask * u_illum_intensity;
                        shadowAmount *= (1.0 - reduction);
                    }
...
class StructuralShadowsLayer extends MaskedEffectLayer {
    constructor() {
        ...
        // --- MODIFICATION START ---
        // Add properties to store the illumination texture and the bound hook callback.
        this.illuminationTexture = null;
        this._onIlluminationBufferReadyBound = this._onIlluminationBufferReady.bind(this);
        // --- MODIFICATION END ---
    ...
    _onIlluminationBufferReady(api) {
        console.log(this.LOG_PREFIX, "Hook 'illuminationBufferReady' fired. Storing texture reference.");
        this.illuminationTexture = api.getLightingTexture();
    }
    ...
    async _draw(options) {
        ...
        // --- MODIFICATION START ---
        // Listen for the illumination buffer.
        Hooks.on('illuminationBufferReady', this._onIlluminationBufferReadyBound);

        // Also check if the buffer is *already* ready to handle race conditions.
        const illuminationAPI = game.modules.get('illuminationbuffer')?.api;
        if (illuminationAPI) {
            console.log(this.LOG_PREFIX, "Found existing Illumination Buffer API during _draw. Storing texture immediately.");
            this._onIlluminationBufferReady(illuminationAPI);
        }
        // --- MODIFICATION END ---
    ...
    _onAnimate(deltaTime) {
        ...
        const wantsIllumination = illumConfig.enabled;

        // NEW: Direct check for illumination buffer readiness on every frame.
        const illuminationAPI = game.modules.get('illuminationbuffer')?.api;
        const illuminationTexture = illuminationAPI?.getLightingTexture();
        const isStale = game.mapShine.illuminationBufferIsStale;
        const isIlluminationReady = wantsIllumination && !isStale && illuminationAPI && illuminationTexture?.valid &&
                                    illuminationTexture.width === Math.round(screen.width) &&
                                    illuminationTexture.height === Math.round(screen.height);
    ...
    async _tearDown(options) {
        console.log(this.LOG_PREFIX, "Tear Down called. Destroying all resources and removing hook listener.");
        
        // --- MODIFICATION START ---
        // Remove the hook listener to prevent memory leaks.
        Hooks.off('illuminationBufferReady', this._onIlluminationBufferReadyBound);
        // --- MODIFICATION END ---
...
class IridescenceLayer extends MaskedEffectLayer {
    ...
    async _draw(options) {
        ...
        const renderer = canvas.app.renderer;
        this.distortionNoiseManager = new NoiseTextureManager(renderer, 'iridescence.distortion', true);

        try {
            this.iridescenceFilter = new IridescenceFilter();
        } catch (e) {
            const errorString = `Failed to compile IridescenceFilter: ${e.message}`;
            console.error(`MapShine | ${errorString}`);
            ui.notifications.error(errorString, { permanent: true });
        }
...
class AmbientLayer extends CanvasLayer {
    ...
    async _draw(options) {
        console.log("AmbientLayer DEBUG | _draw: Called. Layer is being drawn to the canvas.", { options });
        this._destroyed = false;
    ...
    async _tearDown(options) {
        this._destroyed = true;
        console.log("AmbientLayer DEBUG | _tearDown: Called. Tearing down layer.", { options });
        canvas.app.ticker.remove(this._onAnimateBound);
        window.removeEventListener('resize', this._onResizeBound);
        ...
        this.removeChildren().forEach(c => c.destroy({ children: true, texture: true, baseTexture: true }));
        this.effectSprites.clear();
...
class WaterFXLayer extends MaskedEffectLayer {
    ...
    async _draw(options) {
        ...
        try {
            ...
            const errorString = `Failed to compile WaterEffectsFilter: ${e.message}`;
            console.error(`MapShine | ${errorString}`);
            ui.notifications.error(errorString, { permanent: true });
        }
...
class CurveEditor {
    ...
    getNormalizedPoints() {
        ...
        return this.points.map(p => ({ x: p.x / this.width, y: p.y / this.height }));
    }
...
class DebuggerEventHandler {
        ...
        this.element.addEventListener('click', (e) => {
            ...
            if (action === 'open-map-points-editor') {
                ...
                if (!game.mapShine.mapPointsEditor || game.mapShine.mapPointsEditor.closing) {
                    game.mapShine.mapPointsEditor = new MapPointsEditor();
                    game.mapShine.mapPointsEditor.render(true);
                } else {
            ...
            if (preset) {
                ...
                if (newPath && newPath.toLowerCase().endsWith('.cube')) {
                    ...
                    if (lutData && lutData.domain) {
                        await Promise.all([
                            this.profileManager.recordUserChange('postProcessing.lut.domainMin', { r: lutData.domain[0][0], g: lutData.domain[1][0], b: lutData.domain[2][0] }),
                            this.profileManager.recordUserChange('postProcessing.lut.domainMax', { r: lutData.domain[0][1], g: lutData.domain[1][1], b: lutData.domain[2][1] })
                        ]);
                    }
                }
            }
    ...
    _onSaveColorFavorite() {
        ...
        const favorite = {
            ...
            levels: { ...ccConfig.levels },
            whiteBalance: { ...ccConfig.whiteBalance },
            tint: { ...ccConfig.tint },
    ...
    _updateDynamicExposureReadout() {
        const manager = game.mapShine.dynamicExposureManager;
        if (!manager || !manager.isInitialized || !manager.debugData) return;

        const data = manager.debugData;

        const nameEl = this.element.querySelector('#de-token-name');
        const sensedLumEl = this.element.querySelector('#de-current-lum');
        const adaptedLumEl = this.element.querySelector('#de-adapted-lum');
        const currentExpEl = this.element.querySelector('#de-current-exp');
        const currentConEl = this.element.querySelector('#de-current-con');

        if (nameEl) nameEl.textContent = data.activeTokenName ?? "N/A";
        if (sensedLumEl) sensedLumEl.textContent = (data.sensedLuminance ?? 0).toFixed(3);
        if (adaptedLumEl) adaptedLumEl.textContent = (data.adaptedLuminance ?? 0).toFixed(3);
        if (currentExpEl) currentExpEl.textContent = (data.currentExposure ?? 0).toFixed(3);
        if (currentConEl) currentConEl.textContent = (data.currentContrastBoost ?? 1.0).toFixed(3);
    }
...
class MaterialEditorDebugger {
    constructor() {
        ...
        this._boundUpdateReadouts = this._updateReadouts.bind(this);
    }
    ...
    initialize(profileManager) {
        ...
        canvas.app.ticker.add(this._boundUpdateReadouts);

        console.log("Material Editor | UI system initialized.");
    }

    destroy() {
        canvas.app.ticker.remove(this._boundUpdateReadouts);
        this.element?.remove();
        this.element = null;
        this.uiBuilder = null;
        this.eventHandler = null;
    ...
    _updateReadouts() {
        if (this.eventHandler) {
            this.eventHandler._updateDynamicExposureReadout();
        }
    }
...
Hooks.once('init', () => {
    ...
    Hooks.on('lightingRefresh', () => {
        if (canvas?.ready && game.mapShine?.profileManager?.activeConfig) {
            game.mapShine.profileManager.updateAllSystemsFromConfig({ lightingOnly: true });
        }
    });
